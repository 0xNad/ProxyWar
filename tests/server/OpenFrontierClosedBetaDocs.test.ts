...
describe("Open Frontier closed beta docs and commands", () => {
  it("documents the closed beta launch command that package.json exposes", async () => {
    const [packageJson, readinessDoc, remoteDoc, publicDemoDoc, aiLeagueDoc] =
      await Promise.all([
        ...
        fs.readFile(
          path.join(process.cwd(), "docs", "OPEN_FRONTIER_PUBLIC_DEMO.md"),
          "utf8",
        ),
        fs.readFile(
    ...
    expect(scripts["agent:closed-beta"]).toContain("OPEN_FRONTIER_BETA_ENABLED");
    ...
    for (const doc of [readinessDoc, publicDemoDoc, aiLeagueDoc]) {
      ...
      expect(doc).toContain("OPEN_FRONTIER_BETA_CODE");
    }
    ...
    expect(readinessDoc).toContain("artifacts/open-frontier/beta-feedback");
