...
import {
  buildProxyWarMatchPackage,
  proxyWarMatchPackageHtml,
  writeProxyWarMatchPackageArtifacts,
} from "../../src/server/agents/ProxyWarMatchPackage";

describe("ProxyWarMatchPackage", () => {
  it("builds a shareable package around replay, telemetry, logs, comms, and coaching artifacts", () => {
    const matchPackage = buildProxyWarMatchPackage(
      {
        runID: "public-run",
        matchID: "MATCH1",
        scenario: "actions",
  ...
  it("writes JSON, Markdown, and HTML package files", async () => {
    ...
    try {
      const paths = await writeProxyWarMatchPackageArtifacts({
        directory: rootDir,
