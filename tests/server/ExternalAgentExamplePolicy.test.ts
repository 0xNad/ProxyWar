...
function runStarterSmokeTest(endpointUrl: string): Promise<{
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, ["smoke-test.mjs"], {
      ...
      env: {
        ...process.env,
        OPEN_FRONTIER_AGENT_TEST_ENDPOINT_URL: endpointUrl,
        OPEN_FRONTIER_AGENT_TEST_TIMEOUT_MS: "5000",
      },
...
async function listenForSmokeTest(
  ...
  return {
    endpointUrl: `http://127.0.0.1:${address.port}/open-frontier/decide`,
    close: () =>
...
describe("external-agent example policy", () => {
  ...
  it("generates a public Agent Card with the decision endpoint and no secrets", async () => {
    ...
    const card = policy.createAgentCardMarkdown({
      ...
      endpointPath: "/open-frontier/decide",
  ...
  it("generates a health response with Agent Card and decision endpoint metadata", async () => {
    ...
    const health = policy.createHealthResponse({
      ...
      decisionPath: "/open-frontier/decide",
    ...
    expect(health).toMatchObject({
      ...
      decisionEndpointUrl: "https://agent.example.com/open-frontier/decide",
  ...
  it("the runnable starter exposes health, decision, and Agent Card routes", async () => {
    const [source, smokeTest, packageJson] = await Promise.all([
      fs.readFile(
        path.join(
          process.cwd(),
          "examples",
    ...
    expect(source).toContain("/open-frontier/decide");
    ...
    expect(smokeTest).toContain("OPEN_FRONTIER_AGENT_TEST_ENDPOINT_URL");
  ...
  it("starter self-test accepts the selectedLegalActionId health-check contract", async () => {
    ...
    try {
      ...
      expect(capturedBody?.protocolVersion).toBe("open-frontier-agent-v1");
      expect(
  ...
  it("uses AGENT_SKILL.md in the LLM prompt", async () => {
    ...
    expect(prompt).toContain("Open Frontier Agent Skill");
