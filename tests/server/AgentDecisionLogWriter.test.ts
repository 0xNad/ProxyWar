...
describe("AgentDecisionLogWriter", () => {
  it("writes JSONL decisions, match summary, Markdown report, and visual report", async () => {
    ...
    try {
      ...
      expect(report).toContain("# Open Frontier Run run-1");
      ...
      expect(visualReport).toContain("Open real Open Frontier replay renderer");
      ...
      ).resolves.toContain('"packageKind": "open-frontier-match-package"');
      await expect(
      ...
      ).resolves.toContain("Open Frontier Match Package");
      await expect(
