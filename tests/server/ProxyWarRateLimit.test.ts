...
import {
  ...
  ProxyWarRateLimiter,
} from "../../src/server/agents/ProxyWarRateLimit";

describe("ProxyWarRateLimiter", () => {
  it("allows requests up to the configured window limit", () => {
    const limiter = new ProxyWarRateLimiter({ windowMs: 1_000 });
  ...
  it("normalizes persisted snapshots defensively", () => {
    const snapshot = normalizeProxyWarRateLimitSnapshot({
      buckets: [
