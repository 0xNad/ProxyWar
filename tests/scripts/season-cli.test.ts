...
describe("season:* CLIs — real subprocess end to end", () => {
  ...
  function runCli(
    ...
    try {
      const stdout = execFileSync(
        "npx",
        ["tsx", path.join(scriptsDir, scriptName), ...args],
        {
          cwd: repoRoot,
          encoding: "utf8",
  ...
  it("full lifecycle: create -> add-event -> status -> activate -> complete", () => {
    ...
    const statusAfterActivate = runCli("season-status.ts", [
      "--season=season_zero",
    ]);
  ...
  it("refuses to activate an already-active season a second time", () => {
    ...
    const secondActivate = runCli("season-activate.ts", [
      "--season=season_zero",
    ]);
  ...
  describe("season:remove-event", () => {
    ...
    it("removes a present slot, then season:status no longer lists it", () => {
      ...
      const beforeRemoval = runCli("season-status.ts", [
        "--season=season_zero",
      ]);
      expect(beforeRemoval.stdout).toContain("event slots: 1");

      const removed = runCli("season-remove-event.ts", [
        `--season=season_zero`,
        `--featured=${FEAT_ID}`,
      ]);
