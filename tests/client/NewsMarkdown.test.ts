...
describe("normalizeNewsMarkdown", () => {
  it("converts openfront pull request URLs to short markdown links", () => {
    const input =
      "Fix attack logic in https://github.com/0xNad/ProxyWar/pull/1234";

    const result = normalizeNewsMarkdown(input);

    expect(result).toContain(
      "[#1234](https://github.com/0xNad/ProxyWar/pull/1234)",
    );
  });

  it("converts openfront compare URLs to markdown links", () => {
    const input =
      "Full Changelog: https://github.com/0xNad/ProxyWar/compare/v1.0.0...v1.1.0";

    const result = normalizeNewsMarkdown(input);

    expect(result).toContain(
      "[v1.0.0...v1.1.0](https://github.com/0xNad/ProxyWar/compare/v1.0.0...v1.1.0)",
    );
  });
