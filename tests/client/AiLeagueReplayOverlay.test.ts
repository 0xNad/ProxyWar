...
describe("AiLeagueReplayOverlay", () => {
  ...
  it("renders a read-only decision panel for the real Open Frontier replay route", () => {
    ...
    expect(overlay?.textContent).toContain("Open Frontier Replay");
    expect(overlay?.textContent).toContain("build:Defense Post:10");
    expect(overlay?.textContent).toContain(
      "1 Open Frontier agents vs 10 built-in opponents",
    );
    ...
    expect(overlay?.textContent).toContain("real Open Frontier replay renderer");
  ...
  it("adds a read-only replay banner without mutating OpenFront-owned prompt DOM", () => {
    ...
    ).toBe("Replay mode: watching Open Frontier agents");
  });
});
