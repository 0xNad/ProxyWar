...
import "../../../../../src/client/prediction/wagering/page/AccountPage";
import type { PremiereAccountPage } from "../../../../../src/client/prediction/wagering/page/AccountPage";
import { resetLeagueDataCacheForTests } from "../../../../../src/client/prediction/wagering/leagueData";

function mount(): PremiereAccountPage {
  const el = document.createElement("premiere-account-page") as PremiereAccountPage;
...
function leagueData(standings: readonly { playerName: string; policyLabel: string }[]) {
  return {
...
function stubFetch(state: { claims: AccountClaim[] }, standings: unknown[]): void {
  vi.stubGlobal(
    "fetch",
    vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = typeof input === "string" ? input : input.toString();
      if (url.includes("/ai-league-runs/league/data.json")) {
        return { ok: true, json: async () => leagueData(standings as never) } as Response;
      }
      ...
      if (url === "/api/account/claim" && init?.method === "POST") {
        ...
        const existingIndex = state.claims.findIndex((c) => c.lineageSlug === lineageSlug);
        const newClaim = claim(lineageSlug, body.label);
        if (existingIndex === -1) {
          state.claims = [...state.claims, newClaim];
        } else {
          state.claims = state.claims.map((c, i) => (i === existingIndex ? newClaim : c));
        }
        return {
          ok: true,
      ...
      if (url === "/api/account/claim/remove" && init?.method === "POST") {
        ...
        state.claims = state.claims.filter((c) => c.lineageSlug !== body.lineageSlug);
        return {
          ok: true,
          json: async () => ({ schemaVersion: 1, claims: state.claims }),
        } as Response;
...
describe("premiere-account-page claim set", () => {
  it("claiming a second lineage keeps the first — both show in the claimed list", async () => {
    ...
    const submitButton = [...el.querySelectorAll("button")].find(
      (b) => b.textContent?.includes("This is me"),
    );
  ...
  it("removing one claimed lineage keeps the other", async () => {
    const state = { claims: [claim("daveey-proxywar"), claim("second-lineage")] };
    stubFetch(state, [
      { playerName: "Daveey", policyLabel: "daveey-proxywar:v24" },
      { playerName: "Second Agent", policyLabel: "second-lineage:v3" },
    ]);
