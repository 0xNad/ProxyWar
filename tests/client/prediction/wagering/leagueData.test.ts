/**
 * Coverage for the league-context join layer: defensive parsing of the
 * externally-generated `data.json` snapshot, and the seat->standing join
 * plus recent-form/head-to-head derivations the scouting panel renders.
 */
...
} from "../../../../src/client/prediction/wagering/leagueData";
...
describe("parseLeagueData", () => {
  ...
  it("returns null for an unusable top-level shape", () => {
    ...
    expect(parseLeagueData({ standings: [], episodes: "not-an-array" })).toBeNull();
  });

  it("drops individually malformed rows instead of throwing", () => {
    const raw = rawSnapshot({
      standings: [
        { rank: 1, playerName: "daveey", score: 20.5, roundsPlayed: 626, isHouse: false },
        { rank: "not-a-number", playerName: "broken" },
        null,
      ],
      episodes: [
        { episodeRequestId: "ereq_ok", players: [{ name: "daveey", isAlive: true, isWinner: true }] },
...
describe("fetchLeagueData", () => {
  ...
  it("caches across calls within a page load — one request per process", async () => {
    const fetchImpl = vi.fn(async () => ({ ok: true, json: async () => rawSnapshot() })) as unknown as typeof fetch;
