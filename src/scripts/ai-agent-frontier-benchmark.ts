...
async function writeFrontierReplayArtifacts(input: {
  ...
  const spectatorReplay = buildAgentSpectatorReplay({
    ...
    notes: [
      ...
      "Native Open Frontier replay uses the saved game-record.json and read-only replay path.",
    ],
  });
...
function frontierReport(input: {
  ...
  return [
    `# Open Frontier Agent-vs-Nation Benchmark Report`,
    "",
...
function performanceDiagnosis(
  ...
  const lines = [
    "# Open Frontier Full-Match Performance Diagnosis",
    "",
