...
await runCheck("hosted_url", "Hosted URL", async () => {
  if (!options.publicUrl.startsWith("https://")) {
    throw new Error("OPEN_FRONTIER_PUBLIC_URL must be an HTTPS beta URL.");
  }
  if (
    options.publicUrl.includes("example.") ||
    options.publicUrl.includes("your-beta-url")
  ) {
    throw new Error("OPEN_FRONTIER_PUBLIC_URL still looks like a placeholder.");
  }
...
await runCheck("agent_start", "Agent start page", async () => {
  ...
  if (!text.includes("Open Frontier Agent Start")) {
...
function formatReport(report: SmokeReport): string {
  return [
    `Open Frontier hosted beta smoke: ${report.status}`,
...
function parseArgs(args: string[]): SmokeOptions {
  const publicUrl = normalizePublicUrl(
    stringArg(args, "--public-url=") ?? process.env.OPEN_FRONTIER_PUBLIC_URL,
  );
  const inviteCode =
    stringArg(args, "--invite-code=") ??
    process.env.OPEN_FRONTIER_BETA_CODE ??
    process.env.OPEN_FRONTIER_BETA_PASSWORD;
  if (publicUrl === null) {
    throw new Error("Set OPEN_FRONTIER_PUBLIC_URL or pass --public-url=https://...");
  }
  if (inviteCode === undefined || inviteCode.trim() === "") {
    throw new Error("Set OPEN_FRONTIER_BETA_CODE or pass --invite-code=...");
  }
  return {
    publicUrl,
