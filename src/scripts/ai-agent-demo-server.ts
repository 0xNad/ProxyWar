...
} from "../server/agents/OpenFrontierBetaAccess";
import {
...
import type { AgentManifestProvider } from "../server/agents/AgentManifest";
import {
  fetchAndParseOpenFrontierAgentCard,
  normalizeOpenFrontierAgentCardInput,
} from "../server/agents/OpenFrontierAgentCard";
import {
...
import {
  ...
  type OpenFrontierPublicReadinessReport,
} from "../server/agents/OpenFrontierPublicReadiness";
import {
  normalizeOpenFrontierRateLimitSnapshot,
  OpenFrontierRateLimiter,
  type OpenFrontierRateLimitSnapshot,
} from "../server/agents/OpenFrontierRateLimit";

const app = express();
const networkConfig = loadOpenFrontierDemoServerNetworkConfig();
const serverUrls = buildOpenFrontierDemoServerUrls(networkConfig);
...
const configuredNationsRootDir = process.env.OPEN_FRONTIER_NATIONS_DIR?.trim();
...
const betaAccess = loadOpenFrontierBetaAccessConfig();
...
const rateLimiter = new OpenFrontierRateLimiter({
  windowMs: positiveInt(process.env.OPEN_FRONTIER_RATE_LIMIT_WINDOW_MS, 60_000),
...
app.get("/beta", (req, res) => {
  const returnTo = normalizeOpenFrontierBetaReturnTo(queryParam(req.query.next));
  ...
  res
    .type("html")
    .send(renderOpenFrontierBetaLoginHtml(betaAccess, undefined, returnTo));
});
...
app.post("/api/beta/login", (req, res) => {
  ...
  const returnTo = normalizeOpenFrontierBetaReturnTo(
    returnToFromBody(req.body as Record<string, unknown>),
  );
  if (!verifyOpenFrontierBetaInviteCode(betaAccess, inviteCode)) {
    res
      .status(401)
      .type("html")
      .send(
  ...
  const token = createOpenFrontierBetaSessionToken({ inviteCode });
...
app.get("/agent-start", async (_req, res, next) => {
  try {
    ...
    res.type("html").send(renderOpenFrontierAgentStartHtml(model));
  } catch (error) {
    next(error);
  }
});
...
app.get("/agent-start.json", async (_req, res, next) => {
  try {
    ...
    res.json(openFrontierAgentStartJson(model));
  } catch (error) {
    next(error);
  }
});

app.get("/protocol/open-frontier-agent.schema.json", (_req, res) => {
  res.json(openFrontierAgentProtocolSchema());
});
...
app.use((req, res, next) => {
  ...
  if (req.path.startsWith("/api/")) {
    res.status(401).json({ error: "Open Frontier beta invite required" });
    return;
  }
...
app.get("/public", async (_req, res, next) => {
  try {
    ...
    res.type("html").send(renderOpenFrontierPublicHtml(model));
  } catch (error) {
    next(error);
  }
});
...
app.post("/api/tester-dashboard/endpoint-health", async (req, res) => {
  ...
  try {
    const nations = await listOpenFrontierNations(nationsRootDir);
      ...
      try {
        const result = await checkExternalAgentEndpoint(
          normalizeExternalAgentHealthCheckInput({
            ...
            token: providerTokenInput(provider),
...
app.post("/api/jobs", async (req, res) => {
  ...
  try {
    ...
    if (request.roster === "saved") {
      await syncOpenFrontierActiveRoster({ nationsDir: nationsRootDir });
    }
    const queued = enqueueOpenFrontierJob(request);
    if (!queued.ok) {
      res.status(429).json({
        error: queued.error,
        runningJobID,
...
app.post("/api/nations", async (req, res) => {
  ...
  try {
    ...
    const result = await saveOpenFrontierNation(body, { nationsDir: nationsRootDir });
    res.status(201).json({
      nation: {
...
app.delete("/api/nations/:nationID", async (req, res) => {
  ...
  try {
    const result = await deleteOpenFrontierNation(req.params.nationID, {
      nationsDir: nationsRootDir,
    });
    res.json({
      ok: true,
...
app.post("/api/agent-cards/import", async (req, res) => {
  ...
  try {
    const body = req.body as Record<string, unknown>;
    const cardInput = normalizeOpenFrontierAgentCardInput({
      cardUrl: body.cardUrl,
      timeoutMs: body.timeoutMs,
    });
    const card = await fetchAndParseOpenFrontierAgentCard(cardInput);
...
app.post("/api/agent-cards/import-and-run", async (req, res) => {
  ...
  if (runningJobID !== null && queuedJobIDs.length >= maxQueuedJobs) {
    res.status(429).json({
      ok: false,
      error:
        "The Open Frontier demo queue is full. Wait for the current jobs to finish and try again.",
      runningJobID,
      queuedJobCount: queuedJobIDs.length,
    });
    return;
...
app.get("/openfront-replay/:runID", (req, res) => {
  if (!isSafeOpenFrontierArtifactSegment(req.params.runID)) {
...
const server = app.listen(port, host, () => {
  console.log(`Open Frontier demo hub: ${serverUrls.localUrl}`);
  ...
  if (betaAccess.enabled) {
    console.log(`Open Frontier closed beta: ${serverUrls.localUrl}/public`);
    console.log("Invite gate is enabled. The invite code is not printed.");
  }
  console.log(`Open Frontier renderer: ${rendererBaseUrl}`);
...
function hasValidBetaSession(req: Request): boolean {
  ...
  return verifyOpenFrontierBetaSessionToken({
    config: betaAccess,
...
): Promise<OpenFrontierPublicReadinessReport> {
  const model =
    hub ??
...
function providerTokenInput(
  provider: Extract<AgentManifestProvider, { provider: "external-http" }>,
): string | undefined {
...
function enforceRateLimit(
  ...
  res.status(429).json({
    error: "Too many Open Frontier beta requests. Please wait and try again.",
  });
  return false;
}
...
function proxyRendererRequest(
  ...
  if (process.env.AI_LEAGUE_DEMO_RENDERER === "false") {
    res.status(503).send("Open Frontier renderer is not running for this demo server.");
    return;
  }
  if (!isLoopbackRendererBaseUrl(rendererBaseUrl)) {
    res
      .status(503)
      .send("Open Frontier renderer proxy is restricted to a loopback renderer URL.");
    return;
  }
  ...
  proxyReq.on("error", (error) => {
    res
      .status(502)
      .send(`Open Frontier renderer is unavailable: ${error.message}`);
  });
  proxyReq.end();
}
...
function servePublicRunArtifact(
  ...
  if (
    !isSafeOpenFrontierArtifactSegment(runID) ||
    !isOpenFrontierPublicRunArtifact(artifact)
  ) {
...
function servePublicTournamentArtifact(
  ...
  const artifact = stringParam(req.params.artifact);
  if (
    !isSafeOpenFrontierArtifactSegment(tournamentID) ||
    !isOpenFrontierPublicTournamentArtifact(artifact)
  ) {
    res.status(404).send("artifact not available");
    return;
  }
...
function servePublicDoc(req: express.Request, res: express.Response): void {
  const artifact = stringParam(req.params.artifact);
  if (
    !isSafeOpenFrontierArtifactSegment(artifact) ||
    !isOpenFrontierPublicDoc(artifact)
  ) {
...
function servePublicExternalAgentExample(
  ...
  if (
    !isSafeOpenFrontierArtifactSegment(artifact) ||
    !isOpenFrontierPublicExternalAgentExample(artifact)
  ) {
...
function maybeStartRenderer(): ChildProcess | null {
  ...
  child.on("error", (error) => {
    console.error(`Open Frontier renderer failed to start: ${error.message}`);
  });
  child.on("close", (code) => {
    if (code !== 0 && code !== null) {
      console.error(
        `Open Frontier renderer exited with code ${code}. If port ${rendererPort} is already in use, the hub will link to the existing process at ${rendererBaseUrl}.`,
      );
    }
  });
  return child;
...
function startJob(job: AgentDemoJobRecord): void {
  ...
  child.on("close", (code) => {
    ...
    if (code !== 0) {
      job.status = "failed";
      job.errorSummary = failureSummary(job.outputTail);
      void persistJobs().then(() => startNextQueuedJob());
      return;
    }
...
function enqueueOpenFrontierJob(
...
> {
  try {
    return normalizeOpenFrontierRateLimitSnapshot(
...
function failureSummary(outputTail: string): string {
  return (
    outputTail
      .split("\n")
      .map((line) => line.trim())
      .filter(Boolean)
      .slice(-1)[0] ?? "job exited without a clear error line"
  );
}
