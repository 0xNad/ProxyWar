...
const endpointUrl =
  options.endpointUrl ??
  `http://127.0.0.1:${options.port}/open-frontier/decide`;
const children: ChildProcess[] = [];

process.env.OPEN_FRONTIER_ALLOW_PRIVATE_AGENT_ENDPOINTS =
...
try {
  ...
  console.log("Open Frontier external-agent dry run passed", summary);
} finally {
...
function startExampleAgent(port: number): ChildProcess {
  const child = spawn(process.execPath, ["examples/external-agent/simple-agent.mjs"], {
    ...
    env: {
      ...process.env,
      OPEN_FRONTIER_AGENT_HOST: "127.0.0.1",
      OPEN_FRONTIER_AGENT_PORT: String(port),
    },
