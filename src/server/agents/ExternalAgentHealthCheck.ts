...
function healthCheckFixHint(message: string, endpointUrl: string): string {
  ...
  if (
    ...
    lower.includes("open_frontier_agent_llm_provider")
  ) {
    return "Configure the starter model backend first, then restart the endpoint: OPEN_FRONTIER_AGENT_LLM_PROVIDER=codex-cli, claude-cowork, command, or openrouter. Run npm run self-test before saving the agent.";
  }
  ...
  if (lower.includes("open_frontier_agent_llm_command")) {
    return "The command-backed starter needs OPEN_FRONTIER_AGENT_LLM_COMMAND set to a non-interactive command that prints the strict JSON decision. Run npm run self-test after restarting.";
  }
  if (
  ...
  if (lower.includes("content-type")) {
    return url.pathname.endsWith(".md") || url.pathname.includes("agent-card")
      ? "Manual Test Endpoint expects the POST decision endpoint, usually /open-frontier/decide. Use Connect With One Link for /agent-card.md URLs."
  ...
  if (lower.includes("http 404") || lower.includes("not found")) {
    return "Check the endpoint path. The starter decision path is /open-frontier/decide; /agent-card.md is for Agent Card import.";
  }
  if (lower.includes("private") || lower.includes("reserved") || lower.includes("https")) {
    return "Remote beta endpoints must be public HTTPS. For local-only testing, start the host with OPEN_FRONTIER_ALLOW_PRIVATE_AGENT_ENDPOINTS=true.";
  }
  ...
  if (
    ...
    return "Make sure the service is running and reachable from the Open Frontier host, then retry Test Endpoint.";
  }
