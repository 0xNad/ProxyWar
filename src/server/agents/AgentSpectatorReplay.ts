...
export function buildAgentSpectatorReplay(input: {
  ...
  return {
    ...
    notes: [
      ...
      "The native Open Frontier replay client expects archived GameRecord API data; game-record.json is saved as a future integration hook.",
      ...(input.notes ?? []),
    ],
  };
}
...
<head>
  ...
  <title>Open Frontier Spectator ${escapeHtml(replay.runID)}</title>
  <style>
...
<body>
  ...
  <header>
    <h1>Open Frontier Spectator</h1>
    <div class="muted">${escapeHtml(replay.runID)} · read-only artifact replay · spectator occupies no player slot</div>
  </header>
  <main>
    <section class="map-panel">
      <canvas id="map" width="1100" height="720" aria-label="Open Frontier replay map"></canvas>
      <div class="controls">
    ...
    <aside class="side">
      ...
      <section class="panel">
        ...
        <div class="paths">
          ...
          <a href="/ai-league-replay/${encodeURIComponent(replay.runID)}">real Open Frontier renderer</a>
        </div>
        <p class="muted">This viewer is static and read-only: it opens no socket, creates no player, and has no intent submission path. The real Open Frontier renderer link is proxied by the local demo/beta server.</p>
      </section>
    </aside>
  </main>
  <script>
