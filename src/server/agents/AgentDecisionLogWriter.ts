...
import {
  OpenFrontierMatchPackagePaths,
  writeOpenFrontierMatchPackageArtifacts,
} from "./OpenFrontierMatchPackage";
...
export async function writeAgentLeagueRunArtifacts(
  ...
  const matchPackagePaths = await writeOpenFrontierMatchPackageArtifacts({
    directory,
    summary,
  });
  ...
  return [
    ...
    summary.spectator
      ? [
          ...
          `- Native Open Frontier GameRecord hook: ${summary.spectator.gameRecordPath ?? "not written"}`,
          "- This artifact viewer does not open a GameServer socket, does not create a player, and cannot submit intents.",
          `- Real Open Frontier renderer route: /ai-league-replay/${encodeURIComponent(input.runID)}`,
        ].join("\n")
...
function visualReport(
  ...
  paths: {
    ...
    matchPackagePaths: OpenFrontierMatchPackagePaths;
    spectatorTelemetryPath: string;
...
<head>
  ...
  <title>Open Frontier Run ${escapeHtml(input.runID)}</title>
  <style>
...
<body>
  ...
  <main>
    ...
    <section class="panel">
      ...
      ${
        summary.spectator
          ? `<p><a href="/ai-league-replay/${encodeURIComponent(input.runID)}">Open real Open Frontier replay renderer</a></p><p class="notes">Requires the local demo/beta server; this route is proxied for remote viewers.</p>`
          : ""
      }
    </section>
    ...
    <section class="panel">
      ...
      <ul class="paths">
        ...
        <li><a href="/ai-league-replay/${encodeURIComponent(input.runID)}">real Open Frontier replay renderer</a> <code>/ai-league-replay/${escapeHtml(input.runID)}</code></li>
        ${
