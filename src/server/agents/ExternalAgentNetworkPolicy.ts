...
async function resolveEndpointAddresses(
  ...
  const addressHostname = stripIpv6UrlBrackets(hostname);
  if (net.isIP(addressHostname) !== 0) {
    return [
      {
        address: addressHostname,
        family: net.isIP(addressHostname) as 4 | 6,
      },
    ];
  }
  if (addressHostname.toLowerCase() === "localhost") {
    return [
  ...
  if (options.resolveAddresses !== undefined) {
    return normalizeResolvedAddresses(await options.resolveAddresses(addressHostname));
  }
  const records = await dns.lookup(addressHostname, { all: true, verbatim: false });
  return normalizeResolvedAddresses(records);
}

function stripIpv6UrlBrackets(hostname: string): string {
