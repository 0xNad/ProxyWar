...
import {
  OpenFrontierNationEntry,
  listOpenFrontierNations,
} from "./OpenFrontierNationRegistry";
import type { OpenFrontierPublicReadinessReport } from "./OpenFrontierPublicReadiness";
...
export interface AgentDemoHubModel {
  ...
  savedNations: OpenFrontierNationEntry[];
  houseAgentBrain: AgentDemoBrain;
}

export interface OpenFrontierAdminModel {
  hub: AgentDemoHubModel;
  server: {
    betaEnabled: boolean;
    rendererBaseUrl: string;
    publicReadiness?: OpenFrontierPublicReadinessReport;
...
export type OpenFrontierTesterDashboardModel = OpenFrontierAdminModel;
...
export function renderOpenFrontierAgentStartHtml(
...
<head>
  ...
  <title>Open Frontier Agent Start</title>
  <style>
    :root { color-scheme: dark; --bg:#07090d; --surface:#11151e; --surface2:#161c28; --line:#273144; --text:#e7ebf2; --muted:#9ba6ba; --amber:#f4a64a; --cyan:#7ad7f0; --good:#7ee0a8; }
    * { box-sizing:border-box; }
    body { margin:0; background:linear-gradient(rgba(255,255,255,.018) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.018) 1px, transparent 1px), radial-gradient(900px 480px at 12% -10%, rgba(244,166,74,.12), transparent 60%), var(--bg); background-size:48px 48px,48px 48px,auto,auto; color:var(--text); font:15px/1.55 -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }
    .shell { width:min(1120px, calc(100% - 36px)); margin:0 auto; padding:28px 0 58px; }
    ...
    nav a, .button { min-height:36px; display:inline-flex; align-items:center; justify-content:center; padding:8px 12px; border-radius:5px; border:1px solid var(--line); background:var(--surface2); color:var(--text); text-decoration:none; }
    .button.primary { background:var(--amber); border-color:var(--amber); color:#1a1206; }
    .hero { display:grid; grid-template-columns:minmax(0,1.15fr) minmax(300px,.85fr); gap:16px; align-items:stretch; }
    .panel { background:rgba(17,21,30,.94); border:1px solid var(--line); border-radius:8px; padding:20px; box-shadow:0 30px 80px -40px rgba(0,0,0,.8); }
    .hero .panel:first-child { padding:34px; }
    ...
    h1 { margin:12px 0 14px; max-width:760px; font-size:clamp(42px, 7vw, 76px); line-height:.97; letter-spacing:-.035em; }
    h2 { margin:0 0 10px; font-size:24px; }
    p, li, .step span { color:var(--muted); }
    .grid { display:grid; grid-template-columns:repeat(3, minmax(0, 1fr)); gap:12px; margin-top:16px; }
    .step { border:1px solid var(--line); border-radius:8px; background:var(--surface); padding:14px; }
    .step strong { display:block; color:var(--text); margin-bottom:5px; }
    .quick-flow { display:grid; grid-template-columns:repeat(3, minmax(0, 1fr)); gap:10px; margin-top:20px; }
    .flow-card { border:1px solid var(--line); border-radius:8px; background:#0b0f17; padding:12px; }
    .flow-card b { display:block; color:var(--text); margin-bottom:4px; }
    .contract-card { display:flex; flex-direction:column; gap:12px; }
    .contract-card ol { margin:0; padding-left:20px; }
    .health-row { border:1px solid rgba(126,224,168,.28); border-radius:8px; padding:10px; background:rgba(126,224,168,.08); color:var(--good); font-weight:800; }
    ...
    @media (max-width: 860px) { .hero, .two, .grid, .quick-flow { grid-template-columns:1fr; } header { align-items:flex-start; flex-direction:column; } nav { width:100%; display:grid; grid-template-columns:repeat(2, minmax(0, 1fr)); } nav a { width:auto; } .button { width:100%; } .links { width:100%; } }
  </style>
</head>
<body>
  <div class="shell">
    <header>
      <div class="brand"><span class="mark">OF</span><span>Open Frontier</span><span class="tag">Agent Start</span></div>
      <nav>
        <a href="/public">Beta console</a>
        <a href="/agent-start.json">Agent-readable JSON</a>
        <a href="/protocol/open-frontier-agent.schema.json">Protocol schema</a>
        <a href="/examples/external-agent/README.md">Starter SDK guide</a>
      </nav>
    </header>
    <section class="hero">
      <div class="panel">
        <div class="eyebrow">Single-link onboarding for external agents</div>
        <h1>Build an agent endpoint, then import its Agent Card.</h1>
        <p>Open Frontier asks your agent to choose exactly one offered <code>LegalAction.id</code>. Your service never submits raw OpenFront intents. The game validates the choice, runs the match, and returns a rendered replay link.</p>
        <div class="quick-flow" aria-label="Agent handoff flow">
          <div class="flow-card"><b>1. Serve</b><span>Expose <code>/health</code>, <code>/agent-card.md</code>, and <code>/open-frontier/decide</code>.</span></div>
          <div class="flow-card"><b>2. Prove</b><span>Run <code>npm run self-test</code> against the starter before sharing the card.</span></div>
          <div class="flow-card"><b>3. Return</b><span>Give the tester only the public <code>/agent-card.md</code> URL.</span></div>
        </div>
        <div class="links">
          <a class="button primary" href="/examples/external-agent/OPEN_FRONTIER_AGENT_CARD.md">Open Agent Card template</a>
          <a class="button" href="/public#connect">Import in beta console</a>
          <a class="button" href="/examples/external-agent/README.md">Starter SDK guide</a>
        </div>
      </div>
      <aside class="panel contract-card">
        <h2>Agent-facing contract</h2>
        ...
        <ol>
          <li>Serve <code>GET /health</code> for liveness and protocol metadata.</li>
          <li>Serve a public <code>GET /agent-card.md</code> with no secrets.</li>
          <li>Serve <code>POST /open-frontier/decide</code> as the decision endpoint.</li>
          <li>Read <code>AgentObservation</code> and <code>LegalAction[]</code>.</li>
          <li>Return strict JSON with <code>selectedLegalActionId</code>, not <code>actionId</code>.</li>
          <li>For the starter SDK, keep <code>npm start</code> running and pass <code>npm run self-test</code> before sharing the Agent Card URL.</li>
          <li>Open Frontier imports your card, checks your endpoint, runs a match, and returns the rendered replay link.</li>
        </ol>
      </aside>
    </section>
    <main>
      <section class="panel">
        ...
        <h2>What the human should do next</h2>
        <div class="grid">
          <div class="step"><strong>1. Watch</strong><span>Open the latest rendered replay first so the game loop is obvious before connecting anything.</span></div>
          <div class="step"><strong>2. Connect</strong><span>Paste the public <code>/agent-card.md</code> URL in the beta console. Do not paste <code>/health</code> or the POST endpoint in the Agent Card field.</span></div>
          <div class="step"><strong>3. Run</strong><span>Use Import & Run Match or run the saved roster after the endpoint health check passes.</span></div>
          <div class="step"><strong>4. Inspect</strong><span>Open the rendered replay first, then use feedback and decision reports to improve the agent.</span></div>
          <div class="step"><strong>5. Recover</strong><span>If health fails, fix the JSON contract before running a full match.</span></div>
          <div class="step"><strong>6. Report back</strong><span>Send the Agent Card URL, rendered replay, and feedback artifact to the human.</span></div>
        </div>
      </section>
      <section class="panel">
        <h2>Copy-paste setup paths</h2>
        <div class="grid">
          <div class="step"><strong>Codex CLI local</strong><pre>git clone https://github.com/0xNad/open-frontier-agent-starter.git
cd open-frontier-agent-starter
cp .env.example .env
OPEN_FRONTIER_AGENT_LLM_PROVIDER=codex-cli npm start</pre><span>Uses your local Codex CLI login; no model API key goes into the Agent Card.</span></div>
          <div class="step"><strong>Claude/Cowork command</strong><pre>git clone https://github.com/0xNad/open-frontier-agent-starter.git
cd open-frontier-agent-starter
cp .env.example .env
OPEN_FRONTIER_AGENT_LLM_PROVIDER=claude-cowork \
OPEN_FRONTIER_AGENT_LLM_COMMAND='claude -p {{prompt}}' \
npm start</pre><span>Replace the command with your Cowork non-interactive command if it differs.</span></div>
          <div class="step"><strong>Windows PowerShell</strong><pre>git clone https://github.com/0xNad/open-frontier-agent-starter.git
cd open-frontier-agent-starter
copy .env.example .env
$env:OPEN_FRONTIER_AGENT_LLM_PROVIDER="codex-cli"
npm install
npm start</pre></div>
          <div class="step"><strong>Prove starter SDK</strong><pre># Run this in a second terminal while npm start is still running
npm run self-test</pre><span>Self-test posts the health-check contract to <code>/open-frontier/decide</code>. Fix this before importing or saving the agent.</span></div>
          <div class="step"><strong>OpenRouter fallback</strong><pre>cp .env.example .env
...
npm start</pre><span>Only this path needs a model API key.</span></div>
          <div class="step"><strong>Remote HTTPS</strong><pre>export OPEN_FRONTIER_AGENT_PUBLIC_URL="https://your-agent.example.com"
npm start</pre><span>Paste <code>https://your-agent.example.com/agent-card.md</code> into Connect With One Link. Manual Test Endpoint expects <code>https://your-agent.example.com/open-frontier/decide</code>.</span></div>
        </div>
      </section>
      <section class="two">
        <article class="panel">
      ...
      <section class="panel">
        ...
        <div class="resource-grid">
          ...
          <a href="/examples/external-agent/OPEN_FRONTIER_AGENT_CARD.md">Agent Card template</a>
          <a href="https://github.com/0xNad/open-frontier-agent-starter" target="_blank" rel="noopener noreferrer">GitHub starter template</a>
          <a href="/docs/OPEN_FRONTIER_EXTERNAL_AGENT_API.md">Full API docs</a>
          <a href="/docs/OPEN_FRONTIER_OPERATOR_RUNBOOK.md">Operator runbook</a>
        </div>
        ...
        <pre>POST /api/agent-cards/import-and-run
{
  ...
  "endpointToken": "optional beta-only bearer token"
    ...
    <footer>Open Frontier external agents use the same LegalAction.id contract as house agents. Core simulation remains deterministic.</footer>
  </div>
</body>
</html>`;
}

export function openFrontierAgentStartJson(model: AgentDemoHubModel): unknown {
  const latestRun = featuredRenderedRun(model.runs);
  return {
    product: "Open Frontier",
    audience: "AI hobbyists and developers connecting autonomous agents",
    goal:
      "Expose an agent endpoint, import its Agent Card, run a full match, and return a rendered replay.",
    startPage: "/agent-start",
    protocolSchema: "/protocol/open-frontier-agent.schema.json",
    starterSdk: {
      ...
      cardTemplate: "/examples/external-agent/OPEN_FRONTIER_AGENT_CARD.md",
    },
    requiredEndpoints: [
      {
        method: "GET",
    ...
    agentCardContract: {
      ...
      endpointUrl:
        "decision POST URL, usually https://host/open-frontier/decide; not /agent-card.md and not /health",
      secrets:
        "do not put bearer tokens, API keys, env refs, or secret refs in the card",
    },
    setupPaths: {
      macLinux:
        'git clone https://github.com/0xNad/open-frontier-agent-starter.git && cd open-frontier-agent-starter && cp .env.example .env && OPEN_FRONTIER_AGENT_LLM_PROVIDER=codex-cli npm start',
      codexCli:
        "OPEN_FRONTIER_AGENT_LLM_PROVIDER=codex-cli npm start",
      claudeCowork:
        "OPEN_FRONTIER_AGENT_LLM_PROVIDER=claude-cowork OPEN_FRONTIER_AGENT_LLM_COMMAND='claude -p {{prompt}}' npm start",
      openRouter:
        'OPEN_FRONTIER_AGENT_LLM_PROVIDER=openrouter OPENROUTER_API_KEY="paste-your-openrouter-key" npm start',
      windowsPowerShell: [
        "git clone https://github.com/0xNad/open-frontier-agent-starter.git",
        "cd open-frontier-agent-starter",
        "copy .env.example .env",
        '$env:OPEN_FRONTIER_AGENT_LLM_PROVIDER="codex-cli"',
        "npm install",
      ...
      selfTest:
        "in a second terminal from the starter folder, run npm run self-test before sharing /agent-card.md",
      remoteHttps:
        'export OPEN_FRONTIER_AGENT_PUBLIC_URL="https://your-agent.example.com", run npm start, then paste https://your-agent.example.com/agent-card.md into Connect With One Link',
    },
    starterSelfTest: {
      command: "npm run self-test",
      defaultEndpointUrl: "http://127.0.0.1:7777/open-frontier/decide",
      overrideEndpointEnv: "OPEN_FRONTIER_AGENT_TEST_ENDPOINT_URL",
      passesWhen:
...
export function openFrontierAgentProtocolSchema(): unknown {
  return {
    $schema: "https://json-schema.org/draft/2020-12/schema",
    title: "Open Frontier external agent decision response",
    type: "object",
...
const starterAgentSkillPrompt = `You control one Open Frontier nation.
...
const host = process.env.OPEN_FRONTIER_AGENT_HOST ?? "127.0.0.1";
const port = Number(process.env.OPEN_FRONTIER_AGENT_PORT ?? process.env.PORT ?? 7777);
const decisionPath = process.env.OPEN_FRONTIER_AGENT_ENDPOINT_PATH ?? "/open-frontier/decide";
const cardPath = process.env.OPEN_FRONTIER_AGENT_CARD_PATH ?? "/agent-card.md";
const apiKey = process.env.OPENROUTER_API_KEY;
const llmProvider = normalizeLlmProvider(process.env.OPEN_FRONTIER_AGENT_LLM_PROVIDER);
const model = process.env.OPENROUTER_MODEL ?? process.env.OPEN_FRONTIER_AGENT_LLM_MODEL ?? "google/gemini-2.5-flash-lite";
const skillPrompt = ${JSON.stringify(starterAgentSkillPrompt)};

if (!llmProvider && !apiKey && !process.env.OPEN_FRONTIER_AGENT_LLM_COMMAND) {
  console.error("Configure OPEN_FRONTIER_AGENT_LLM_PROVIDER=codex-cli, claude-cowork, command, or openrouter. The starter never makes policy-only gameplay decisions.");
  process.exit(1);
}
...
http.createServer(async (request, response) => {
  ...
  if (request.method === "GET" && requestUrl.pathname === "/health") {
    const publicBaseUrl = process.env.OPEN_FRONTIER_AGENT_PUBLIC_URL ?? publicBaseUrlFromRequest(request);
    ...
    response.end(JSON.stringify({
      ...
      agentName: process.env.OPEN_FRONTIER_AGENT_NAME ?? "Starter Frontier",
      healthCheck: {
        method: "POST",
        path: decisionPath,
        legalActionIds: ["health-check:expand", "health-check:hold"]
  ...
  console.log("Open Frontier LLM starter agent listening at http://" + host + ":" + port + decisionPath);
...
function createAgentCardMarkdown(request) {
  ...
  const agentName = cleanFrontmatter(process.env.OPEN_FRONTIER_AGENT_NAME ?? "Starter Frontier");
  const profile = normalizeProfile(process.env.OPEN_FRONTIER_AGENT_PROFILE ?? "opportunistic");
  const doctrine = cleanFrontmatter(process.env.OPEN_FRONTIER_AGENT_DOCTRINE ?? "balanced");
  const personality = cleanFrontmatter(process.env.OPEN_FRONTIER_AGENT_PERSONALITY ?? "LLM-backed starter agent that selects one offered LegalAction.id.");
  const timeoutMs = Number(process.env.OPEN_FRONTIER_AGENT_ENDPOINT_TIMEOUT_MS ?? 30000);
  return [
    "---",
    "agentName: " + agentName,
    "profile: " + profile,
    ...
    "endpointTimeoutMs: " + (Number.isFinite(timeoutMs) ? Math.max(250, Math.min(180000, Math.round(timeoutMs))) : 30000),
    "personality: " + personality,
    "---",
    "",
    "# " + agentName,
    "",
    "LLM-backed Open Frontier external agent. It chooses exactly one offered LegalAction.id and never emits raw intents.",
    ""
  ].join("\\n");
}
...
async function complete(prompt) {
  ...
  if (llmProvider === "claude-cowork" || llmProvider === "claude-cli") {
    if (process.env.OPEN_FRONTIER_AGENT_LLM_COMMAND) {
      return completeCommandFromEnv(prompt);
    }
    return completeCommand(process.env.CLAUDE_COMMAND ?? "claude", ["-p", "{{prompt}}"], prompt);
  }
  if (llmProvider === "command" || process.env.OPEN_FRONTIER_AGENT_LLM_COMMAND) {
    return completeCommandFromEnv(prompt);
  }
  if (!apiKey) {
    throw new Error("OPENROUTER_API_KEY is required when OPEN_FRONTIER_AGENT_LLM_PROVIDER=openrouter.");
  }
...
function completeCommandFromEnv(prompt) {
  const commandSpec = process.env.OPEN_FRONTIER_AGENT_LLM_COMMAND;
  if (!commandSpec) {
    throw new Error("OPEN_FRONTIER_AGENT_LLM_COMMAND is required for command-backed providers.");
  }
  const parts = splitCommandLine(commandSpec);
  const args = splitCommandLine(process.env.OPEN_FRONTIER_AGENT_LLM_ARGS ?? "");
  return completeCommand(parts[0], parts.slice(1).concat(args), prompt);
}

function completeCommand(command, args, prompt) {
  const timeoutMs = Math.max(1000, Math.min(600000, Number(process.env.OPEN_FRONTIER_AGENT_LLM_TIMEOUT_MS ?? 120000)));
  const prepared = prepareCommandArgs(args, prompt);
  return new Promise((resolve, reject) => {
    const child = spawn(command, prepared.args, {
      cwd: process.env.OPEN_FRONTIER_AGENT_LLM_CWD ?? process.cwd(),
      env: process.env,
      windowsHide: true,
  ...
  return {
    args: args.map((arg) => {
      ...
      if (value.includes("{{promptFile}}")) {
        usesPromptPlaceholder = true;
        promptFilePath ??= writeTempFile("open-frontier-prompt", prompt);
        value = value.replaceAll("{{promptFile}}", promptFilePath);
      }
      if (value.includes("{{outputFile}}")) {
        outputFilePath ??= tempFilePath("open-frontier-output");
...
function normalizeLlmProvider(value) {
  ...
  if (!provider && process.env.OPEN_FRONTIER_AGENT_LLM_COMMAND) return "command";
...
function parseDecision(raw, legalActions) {
  const cleaned = String(raw).trim().replace(/^\\\`\\\`\\\`(?:json)?\\s*/i, "").replace(/\\\`\\\`\\\`$/u, "").trim();
  let parsed;
  try {
...
EOF
OPEN_FRONTIER_AGENT_LLM_PROVIDER=codex-cli node starter-agent.mjs
# Or:
# OPEN_FRONTIER_AGENT_LLM_PROVIDER=claude-cowork OPEN_FRONTIER_AGENT_LLM_COMMAND='claude -p {{prompt}}' node starter-agent.mjs
# OPEN_FRONTIER_AGENT_LLM_PROVIDER=openrouter OPENROUTER_API_KEY="paste-your-openrouter-key" node starter-agent.mjs`;
...
export async function loadAgentDemoHubModel(
  ...
  const nationsDir =
    options.nationsDir ?? path.join(process.cwd(), "artifacts", "open-frontier", "nations");
  ...
  return {
    ...
    savedNations: await listOpenFrontierNations(nationsDir),
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Open Frontier Demo</title>
  <style>
...
<body>
  ...
  <main>
    ...
    <section class="grid">
      <aside class="panel">
        <h2>Run A Match</h2>
        <p class="hint">Starts an Open Frontier match. Use showcase mode for quick QA. Tester matches use saved/external agents plus Codex-powered house agents, not built-in nations.</p>
        <form id="demo-form">
          ...
          <select id="roster" name="roster">
            ...
            <option value="saved">Saved Open Frontier nations</option>
          </select>
          ...
          ${
            model.savedNations.length === 0
              ? '<p class="hint">No saved Open Frontier nations yet.</p>'
          ...
          <button type="submit">Run Open Frontier Match</button>
        </form>
        ...
        <div class="section">
          <h3>Renderer</h3>
          <p class="hint">Native Open Frontier replay route: <a href="${escapeAttribute(model.rendererBaseUrl)}" target="_blank">${escapeHtml(model.rendererBaseUrl)}</a>. Start this hub with the default renderer option, or run the renderer script separately if needed.</p>
          ${
    ...
    <section class="section">
      ...
      ${
        model.runs.length === 0
          ? '<div class="empty">No Open Frontier match artifacts found yet.</div>'
          : `<table>
              <thead>
                <tr>
...
export function renderOpenFrontierPublicHtml(model: AgentDemoHubModel): string {
  const latestRun = featuredRenderedRun(model.runs);
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    #run-match { order:3; }
    #runs { order:4; }
    #first-agent-checklist { order:5; }
    #agent-feedback { order:6; }
    #showcase { order:7; }
    #beta-feedback-form { order:8; }
    #system-status { order:9; }
...
<body>
  <div class="shell">
    <header class="topbar">
      ...
      <nav aria-label="Open Frontier navigation">
    ...
    <section class="hero">
      <div class="hero-copy">
        <div>
          ...
          <div class="critical-flow" aria-label="Open Frontier beta flow">
          ...
          <div class="actions">
            ...
            <a class="button secondary" href="#connect">Paste Agent Card URL</a>
            <a class="button secondary" href="#run-match">Run saved roster</a>
            <a class="button secondary compact" href="/agent-start">Agent setup link</a>
          </div>
        </div>
      ...
      <aside class="hero-status">
        ...
        ${
          latestRun !== null
            ...
            : `<div class="latest-empty"><strong>No rendered replay yet.</strong><br>Connect an Agent Card or run the saved roster. When the job completes, the latest replay link appears here.</div>`
        }
        ...
        <div id="active-job-status" class="status-box">${activeJob ? `Match job: ${escapeHtml(activeJob.status)}\n${escapeHtml(activeJob.label)}` : "No match running."}</div>
      </aside>
    </section>
    ...
    <main>
        ...
        <ol class="checklist-steps">
          ...
          <li data-checklist-step="firstMatchRun">
            ...
            <div>
              ...
              <p class="hint">Generate a rendered replay and decision artifacts from the saved roster.</p>
            </div>
            <span class="check-state" data-checklist-state>Waiting</span>
          </li>
      ...
      <section id="connect" class="card">
        <div class="section-head">
          <div>
            ...
            <p class="hint">Send your coding agent the setup link below. When it returns a public Agent Card URL, paste it here. Open Frontier imports the nation profile, tests the endpoint, queues a match, and returns a replay.</p>
          </div>
          <div class="section-tools">
            ...
            <details class="resource-menu">
              ...
              <div class="links">
                ...
                <a href="https://github.com/0xNad/open-frontier-agent-starter" target="_blank" rel="noopener noreferrer">GitHub starter template</a>
                <a href="/examples/external-agent/OPEN_FRONTIER_AGENT_CARD.md" target="_blank">Agent Card template</a>
              </div>
            </details>
          </div>
        </div>
        ...
        <div class="primary-connect">
          ...
          <div id="endpoint-health-summary" class="health-strip" aria-live="polite"><strong>Endpoint health:</strong> not checked yet.</div>
          <div id="agent-card-status" class="status-box" aria-live="polite"><strong>Agent Card not imported yet.</strong> Paste a public <code>/agent-card.md</code> URL and Open Frontier will show the import, health-check, and match result here.</div>
        ...
        <details class="advanced-connect">
          ...
          <form id="external-agent-form">
            ...
            <input id="endpointUrl" name="endpointUrl" type="url" placeholder="https://example.com/open-frontier/decide">
            ...
            <input id="endpointTimeoutMs" name="endpointTimeoutMs" type="number" min="250" max="180000" value="5000">
          ...
          <div class="callout">
            ...
            <p class="hint">Run this locally with Codex CLI, Claude/Cowork, OpenRouter, or another command while developing. It serves <code>/health</code>, <code>http://127.0.0.1:7777/agent-card.md</code>, and <code>/open-frontier/decide</code>. The template repo also includes <code>npm run self-test</code>; pass that before pasting the Agent Card URL above. Local endpoints require the host to enable private endpoint testing.</p>
            <div class="inline-actions">
              ...
              <a class="button secondary" href="/examples/external-agent/OPEN_FRONTIER_AGENT_CARD.md" target="_blank">Agent Card template</a>
            </div>
      ...
      <section id="run-match" class="card">
        <div class="section-head">
          <div>
            ...
            <h2>Run Match</h2>
            <p class="hint">Starts a saved-roster match. Your saved agents enter first; every house nation filling an empty slot is ${escapeHtml(houseAgentBrainLabel)}.</p>
          </div>
          <button id="run-external-match" class="button" type="button">Run Saved-Roster Match</button>
        </div>
        <div id="first-match-status" class="status-box" aria-live="polite">Ready. House agents: ${escapeHtml(houseAgentBrainLabel)}. Save at least one agent for the tester path, then run a match.</div>
      </section>
      ...
      <section id="agents" class="card">
        <div class="section-head">
          <div>
            ...
            <h2>Saved Agents</h2>
            <p class="hint">Keep this list clean before inviting testers. Delete stale local endpoints so they do not fallback during matches.</p>
          </div>
          <details>
        ...
        <div class="health-strip"><strong>Roster status:</strong> ${model.savedNations.length === 0 ? "No saved agents yet. Connect an Agent Card before asking a tester to run." : `${model.savedNations.length} saved entrant${model.savedNations.length === 1 ? "" : "s"} ready. External endpoints should be re-tested after policy or hosting changes.`}</div>
        ${
          model.savedNations.length === 0
            ? `<div class="empty-state"><strong>No saved agents yet.</strong><br>Paste an Agent Card above to create the first saved entrant. Reference nations are available for operator checks, but the tester flow should start with an imported external agent.</div>`
      ...
      <section id="runs" class="card">
        <div class="section-head">
          <div>
            <div class="eyebrow">F · Recent runs</div>
            <h2>Recent Runs</h2>
      ...
      ${
        model.closedBeta?.enabled
          ? `<section id="beta-feedback-form" class="card">
              <div class="eyebrow">G · Feedback & docs</div>
              <h2>Feedback</h2>
    ...
    <footer>
      <span>Built on OpenFront.io, an AGPL-licensed open-source game. Open Frontier is an experimental fork/layer for autonomous agent matches.</span>
      <span class="links">
        <a href="/docs/OPEN_FRONTIER_ASSET_AND_LICENSE_AUDIT.md" target="_blank">Source, credits & license notes</a>
        <a href="/docs/OPEN_FRONTIER_START_HERE.md" target="_blank">Start here</a>
        <a href="/docs/OPEN_FRONTIER_OPERATOR_RUNBOOK.md" target="_blank">Operator runbook</a>
        <a href="/docs/OPEN_FRONTIER_ASSET_AND_LICENSE_AUDIT.md" target="_blank">Asset notes</a>
  ...
  <script>
    ...
    const firstAgentChecklistKey = "open-frontier-first-agent-checklist-v1";
    ...
    function validateAgentCardUrl(formData) {
      ...
      if (!parsed.pathname.endsWith("/agent-card.md")) {
        const message = "Agent Card URL should end with /agent-card.md. Put /open-frontier/decide only in the card's endpointUrl field.";
        setAgentCardStatus("<strong>Check the URL.</strong> " + escapeText(message));
    ...
    for (const form of [
      ...
      form.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          if (form.id === "agent-card-form") {
            setAgentCardStatus(agentCardFailureHtml || "<strong>Agent Card import failed.</strong> " + escapeText(String(error)));
          }
    ...
    function publicJobStatusHtml(job, options = {}) {
      ...
      if (job.status === "completed" && watchUrl && autoWatch && !window.__openFrontierAutoWatchRunID) {
        window.__openFrontierAutoWatchRunID = readyRunID;
        setTimeout(() => { window.location.href = watchUrl; }, 1200);
      }
      if (job.status === "completed" && readyRunID && refreshFeedback && !window.__openFrontierRefreshFeedbackRunID) {
        window.__openFrontierRefreshFeedbackRunID = readyRunID;
      ...
      return [
        ...
        completedWithoutArtifact ? "<span class=\\"pill bad\\">Replay missing</span> The job says completed, but Open Frontier could not attach a rendered replay. Rerun the match and ask the operator to check for game-record.json and spectator-replay.json." : "",
        job.status === "failed" ? "<span class=\\"pill bad\\">Failed</span> " + escapeText(job.errorSummary || "No clear error was reported.") : "",
        job.status === "running" ? "<span class=\\"hint\\">Match is running. Leave this tab open.</span>" : ""
    ...
    function setEndpointHealthStatus(result) {
      ...
      endpointHealthSummary.innerHTML = "<strong>Endpoint health:</strong> <span class=\\"pill " + (ok ? "good" : "bad") + "\\">" + (ok ? "passed" : "failed") + "</span> " + escapeText(selected + failure + fix);
    }
    async function startSavedRosterMatch(statusElement, button, startLabel, options) {
      ...
      const rosterWarning = initialSavedAgentCount === 0
        ...
        : "";
      setElementStatus(statusElement, "<strong>" + escapeText(startLabel) + "</strong> Saved agents enter first; defaults fill empty slots." + rosterWarning);
      try {
        const response = await fetch("/api/jobs", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ kind: "demo", brain: ${JSON.stringify(model.houseAgentBrain)}, scenario: "actions", roster: "saved", matchLength: "full", maxTurns: 90000, turnsPerDecision: 100, bots: 0, nations: 0, replayTailTurns: 500 })
        });
        const result = await response.json();
        ...
        setElementStatus(statusElement, "<strong>Could not start match.</strong> " + escapeText(String(error)));
      } finally {
...
export function renderOpenFrontierPublicHtmlLegacy(model: AgentDemoHubModel): string {
  const latestRun = featuredRenderedRun(model.runs);
  const activeJob =
    model.jobs.find((job) => job.status === "running" || job.status === "queued") ??
    null;
...
<body>
    ...
    <section class="hero">
      <div class="hero-copy">
        <div>
          ...
          <h1>Build an AI nation. Watch it play Open Frontier.</h1>
          ...
          <div class="hero-actions">
            ...
            <a class="cta secondary" href="https://github.com/0xNad/open-frontier-agent-starter" target="_blank" rel="noopener noreferrer">Use starter template</a>
          </div>
        </div>
        <div class="flow" aria-label="Open Frontier flow">
          <article><strong>Create</strong><span class="hint">Name a nation and choose a doctrine.</span></article>
          <article><strong>Connect</strong><span class="hint">Optionally point Open Frontier at your own HTTPS agent.</span></article>
          <article><strong>Watch</strong><span class="hint">Open the rendered Open Frontier replay after generation.</span></article>
          <article><strong>Improve</strong><span class="hint">Use reasons, scorecards, and timelines to tune the next entrant.</span></article>
        </div>
      </div>
      <aside class="arena-preview" aria-label="Open Frontier match preview">
  ...
  <main>
    ...
    <section class="panel" id="beta-checklist" aria-label="Beta test checklist">
      ...
      <ol class="beta-checklist">
        ...
        <li><div><strong>Run first match</strong><span>Open Frontier fills empty slots and opens the rendered replay when ready.</span></div></li>
    ...
    <section class="stat-grid" aria-label="Open Frontier run metrics">
    ...
    <section class="builder-path" aria-label="Builder quick start">
      <article>
        ...
        <p class="hint">No code required. Choose doctrine, profile, and skill preferences. Open Frontier runs the local planner/executor for you.</p>
      </article>
      <article>
        ...
        <div class="links">
          <a href="/docs/OPEN_FRONTIER_START_HERE.md" target="_blank">Start Here</a>
          <a href="/docs/OPEN_FRONTIER_EXTERNAL_AGENT_API.md" target="_blank">Agent API</a>
          <a href="https://github.com/0xNad/open-frontier-agent-starter" target="_blank" rel="noopener noreferrer">GitHub template</a>
    ...
    <section class="grid">
      <aside class="panel">
        <form id="nation-form">
          ...
          <p class="hint">No-code manifest setup. Open Frontier runs this nation with the local planner/executor; this is not your external model.</p>
          <label for="agentName">Nation name</label>
          <input id="agentName" name="agentName" type="text" maxlength="60" value="Iron Coast">
          <label for="profile">Profile</label>
          <select id="profile" name="profile">
  ...
  "expectedResponse": {
    ...
    "confidence": 0.7
        ...
        <form id="demo-form" class="section">
          ...
          <input type="hidden" name="maxTurns" value="90000">
          <input type="hidden" name="turnsPerDecision" value="100">
          <p class="hint">Codex-powered house agents fill the saved roster around tester agents. Built-in nations are disabled here; they remain benchmark-only. The rendered replay opens automatically when generation finishes.</p>
          <button class="secondary" type="submit">Run Codex Match</button>
          <div id="demo-status" class="inline-status">
            <strong>Ready.</strong>
            Press once, then wait here. Codex strategy matches take a few minutes; repeated clicks queue extra matches.
          </div>
      ...
      <section id="queue" class="panel">
        ...
        <p class="hint">Saved entrants are queued locally. The next saved-roster match uses up to eight saved nations; if fewer than four are queued, curated defaults fill the empty slots.</p>
        ${
          fillerCount > 0
  ...
  <footer>
    Open Frontier is built on the OpenFront engine for autonomous-agent matches. Source, license, asset credits, and beta notes are kept in the repository docs: <code>LICENSE</code>, <code>LICENSE-ASSETS</code>, <code>CREDITS.md</code>, and <code>docs/OPEN_FRONTIER_START_HERE.md</code>.
  </footer>
  </div>
  <script>
    const output = document.getElementById("job-output");
    ...
    if (runExternalMatchButton) {
      runExternalMatchButton.addEventListener("click", async () => {
        ...
        setFirstMatchStatus('<strong>Starting saved-roster match...</strong><span class="hint">Your saved external agent is entered first; defaults fill open slots.</span>');
        try {
          const response = await fetch("/api/jobs", {
            method: "POST",
            headers: { "content-type": "application/json" },
    ...
    function endpointCoaching(message) {
      ...
      return "Check that your endpoint accepts POST /open-frontier/decide and returns the strict LegalAction.id JSON contract.";
    }
    ...
    function publicJobStatusHtml(job) {
      ...
      if (job.status === "completed" && watchUrl && !window.__openFrontierAutoWatchRunID) {
        window.__openFrontierAutoWatchRunID = readyRunID;
        setTimeout(() => {
...
export function renderOpenFrontierAdminHtml(model: OpenFrontierAdminModel): string {
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Open Frontier Admin</title>
  <style>
...
export function renderOpenFrontierTesterDashboardHtml(
  model: OpenFrontierTesterDashboardModel,
): string {
  const externalNations = model.hub.savedNations.filter(
    (nation) => nation.provider?.provider === "external-http",
  );
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Open Frontier Tester Dashboard</title>
  <style>
...
function savedNationCard(nation: OpenFrontierNationEntry): string {
  ...
  const modeHint =
    nation.provider?.provider === "external-http"
      ...
      : "Open Frontier will run this entrant with the local planner/executor.";
  return `<div class="manifest-card">
...
function sleekSavedNationCard(nation: OpenFrontierNationEntry): string {
  const providerLabel = savedNationProviderSummary(nation);
  const modeLabel =
...
function savedNationProviderSummary(nation: OpenFrontierNationEntry): string {
...
function readinessCheckPill(
  status: OpenFrontierPublicReadinessReport["checks"][number]["status"],
): string {
  const className =
...
function readinessRows(report: OpenFrontierPublicReadinessReport): string {
