...
export type OpenFrontierMatchPackageArtifactKind =
  | "rendered_replay"
  | "demo_replay"
  | "replay_data"
  | "telemetry"
...
export type OpenFrontierMatchPackageArtifactAudience =
  | "spectator"
  | "builder"
  | "coach"
  | "technical";

export interface OpenFrontierMatchPackageArtifact {
  kind: OpenFrontierMatchPackageArtifactKind;
  label: string;
  href: string;
  audience: OpenFrontierMatchPackageArtifactAudience;
  present: boolean;
  description: string;
}

export interface OpenFrontierMatchPackageSummary {
  runID: string;
  matchID?: string;
  scenario?: string;
  brainMode?: string;
...
export interface OpenFrontierMatchPackage {
  schemaVersion: 1;
  ...
  artifacts: OpenFrontierMatchPackageArtifact[];
  recommendedOpenOrder: OpenFrontierMatchPackageArtifactKind[];
  protocolBoundary: string;
}

export interface OpenFrontierMatchPackagePaths {
  jsonPath: string;
  markdownPath: string;
  htmlPath: string;
}

export function buildOpenFrontierMatchPackage(
  summary: OpenFrontierMatchPackageSummary,
  now: Date = new Date(),
): OpenFrontierMatchPackage {
  ...
  const artifacts = [
    artifact({
      ...
      label: "Rendered Open Frontier replay",
      href: renderedReplayUrl,
      audience: "spectator",
      present: hasGameRecord,
      description: "The primary watch artifact: the match rendered in Open Frontier.",
    }),
    artifact({
      kind: "demo_replay",
      ...
      audience: "spectator",
      present: hasGameRecord,
      description: "The same replay through the Open Frontier demo/beta hub.",
    }),
    artifact({
      kind: "match_story",
    ...
    artifact({
      ...
      audience: "technical",
      present: hasGameRecord,
      description: "Turn stream consumed by the native Open Frontier replay renderer.",
    }),
    artifact({
      kind: "summary",
  ...
  return {
    ...
    title: `Open Frontier Match ${summary.runID}`,
    routes: {
...
export async function writeOpenFrontierMatchPackageArtifacts(input: {
  directory: string;
  summary: OpenFrontierMatchPackageSummary;
  now?: Date;
}): Promise<OpenFrontierMatchPackagePaths> {
  const matchPackage = buildOpenFrontierMatchPackage(input.summary, input.now);
  ...
  await fs.writeFile(markdownPath, openFrontierMatchPackageMarkdown(matchPackage));
  await fs.writeFile(htmlPath, openFrontierMatchPackageHtml(matchPackage));
  return { jsonPath, markdownPath, htmlPath };
}

export function openFrontierMatchPackageHtml(
  matchPackage: OpenFrontierMatchPackage,
): string {
  ...
  const openOrder = matchPackage.recommendedOpenOrder
    ...
    .filter(
      (artifactItem): artifactItem is OpenFrontierMatchPackageArtifact =>
  ...
  <div class="shell">
    <header class="topbar">
      <div class="brand">
        ...
        <span>Open Frontier Match Package</span>
      </div>
    ...
    <section class="hero">
      <div class="hero-copy">
        <div>
          ...
          <h1>Match Package</h1>
          <p class="lede"><code>${escapeHtml(matchPackage.runID)}</code><br>${escapeHtml(matchPackage.matchID ?? "Saved Open Frontier match")} · generated ${escapeHtml(matchPackage.generatedAt)}</p>
        </div>
        <div class="actions">
          ${
...
export function openFrontierMatchPackageMarkdown(
  matchPackage: OpenFrontierMatchPackage,
): string {
  const presentArtifacts = matchPackage.artifacts.filter(
    (artifactItem) => artifactItem.present,
...
function artifact(
  value: OpenFrontierMatchPackageArtifact,
): OpenFrontierMatchPackageArtifact {
  return value;
}
...
function artifactCard(artifactItem: OpenFrontierMatchPackageArtifact): string {
