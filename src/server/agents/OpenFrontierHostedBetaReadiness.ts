import type {
  OpenFrontierPublicReadinessCheckStatus,
  OpenFrontierPublicReadinessReport,
  OpenFrontierPublicReadinessStatus,
} from "./OpenFrontierPublicReadiness";

export interface OpenFrontierHostedBetaReadinessCheck {
  id: string;
  label: string;
...
export interface OpenFrontierHostedBetaReadinessReport {
  status: OpenFrontierPublicReadinessStatus;
  generatedAt: string;
  shareUrl: string;
  gitCommit: string | null;
  checks: OpenFrontierHostedBetaReadinessCheck[];
  nextActions: string[];
  publicReadiness: OpenFrontierPublicReadinessReport;
}

export interface OpenFrontierHostedBetaReadinessInput {
  publicReadiness: OpenFrontierPublicReadinessReport;
...
export function buildOpenFrontierHostedBetaReadinessReport(
  input: OpenFrontierHostedBetaReadinessInput,
): OpenFrontierHostedBetaReadinessReport {
  const checks: OpenFrontierHostedBetaReadinessCheck[] = [
...
export function formatOpenFrontierHostedBetaReadinessReport(
  report: OpenFrontierHostedBetaReadinessReport,
): string {
  return [
    `Open Frontier hosted beta readiness: ${report.status}`,
...
function publicReadinessCheck(
  publicReadiness: OpenFrontierPublicReadinessReport,
): OpenFrontierHostedBetaReadinessCheck {
  if (publicReadiness.status === "blocked") {
    return {
      id: "public_readiness",
      label: "Public readiness",
      status: "fail",
      message: "The base public readiness report is blocked.",
    };
  }
  if (publicReadiness.status === "warning") {
    return {
      ...
      label: "Public readiness",
      status: "warn",
      message: "The base public readiness report has warnings.",
    };
  }
  return {
    id: "public_readiness",
...
function hostedUrlCheck(
  ...
  if (publicUrl === null) {
    return {
      ...
      label: "Hosted URL",
      status: "fail",
      message: "Set OPEN_FRONTIER_PUBLIC_URL to the HTTPS beta domain.",
    };
  }
...
function privateEndpointLockCheck(
  ...
  return allowPrivateAgentEndpoints
    ? {
        ...
        status: "fail",
        message:
          "Disable OPEN_FRONTIER_ALLOW_PRIVATE_AGENT_ENDPOINTS for hosted testers.",
      }
    : {
        id: "private_endpoint_lock",
        label: "External endpoint lock",
...
function houseAgentBrainCheck(
  ...
  return {
    ...
    status: "fail",
    message:
      "House agents must be LLM-backed. Set OPEN_FRONTIER_HOUSE_AGENT_BRAIN=planner-codex-cli before sharing the beta.",
  };
}
...
function queueLimitCheck(maxQueuedJobs: number): OpenFrontierHostedBetaReadinessCheck {
  if (!Number.isInteger(maxQueuedJobs) || maxQueuedJobs < 0) {
    return {
      id: "queue_limit",
      label: "Queue limit",
      status: "fail",
      message: "OPEN_FRONTIER_MAX_QUEUED_JOBS must be a non-negative integer.",
    };
  }
  if (maxQueuedJobs > 1) {
    return {
      ...
      status: "warn",
      message:
        "For first testers, set OPEN_FRONTIER_MAX_QUEUED_JOBS=1 so match jobs cannot pile up.",
    };
  }
  return {
    id: "queue_limit",
...
function persistenceCheck(
  paths: OpenFrontierHostedBetaReadinessInput["paths"],
): OpenFrontierHostedBetaReadinessCheck {
  const failed = [
...
function backupCheck(
  ...
  if (!paths.backupRootConfigured) {
    return {
      ...
      status: "warn",
      message:
        "Backups can use the default local directory. Set OPEN_FRONTIER_BACKUP_DIR on a hosted server.",
    };
  }
  return {
    id: "backup",
...
function onboardingFilesCheck(
  requiredFiles: OpenFrontierHostedBetaReadinessInput["requiredFiles"],
): OpenFrontierHostedBetaReadinessCheck {
  const missing = [
...
function rollbackCheck(
  git: OpenFrontierHostedBetaReadinessInput["git"],
): OpenFrontierHostedBetaReadinessCheck {
...
function nextActions(checks: OpenFrontierHostedBetaReadinessCheck[]): string[] {
