...
const DEFAULT_COOKIE_NAME = "proxywar_beta";
const DEFAULT_SESSION_TTL_MS = 7 * 24 * 60 * 60 * 1000;
const MAX_CLOCK_SKEW_MS = 5 * 60 * 1000;

export interface ProxyWarBetaAccessConfig {
  enabled: boolean;
...
export interface ProxyWarBetaFeedbackEntry {
  feedbackID: string;
  createdAt: string;
...
export function loadProxyWarBetaAccessConfig(
...
export function createProxyWarBetaSessionToken(input: {
  inviteCode: string;
  issuedAtMs?: number;
}): string {
...
export function verifyProxyWarBetaSessionToken(input: {
  config: ProxyWarBetaAccessConfig;
...
export function verifyProxyWarBetaInviteCode(
  config: ProxyWarBetaAccessConfig,
...
export function normalizeProxyWarBetaReturnTo(
...
export function renderProxyWarBetaLoginHtml(
  config: ProxyWarBetaAccessConfig,
  error?: string,
  returnTo?: string,
): string {
  const normalizedReturnTo = normalizeProxyWarBetaReturnTo(returnTo);
  const setupWarning =
    config.enabled && config.inviteCode === null
      ? `<div class="warning">This beta server needs <code>PROXYWAR_BETA_CODE</code> before testers can enter.</div>`
      : "";
  const errorHtml =
...
export function normalizeProxyWarBetaFeedback(
  input: Record<string, unknown>,
  now = new Date(),
): ProxyWarBetaFeedbackEntry {
...
function normalizeRating(
  value: unknown,
): ProxyWarBetaFeedbackEntry["rating"] {
  if (typeof value !== "string") return null;
  if (
    value === "great" ||
    value === "okay" ||
