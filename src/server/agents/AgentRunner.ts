...
export class AgentRunner {
  ...
  submitLegalAction(action: LegalAction): AgentIntentResult {
    ...
    if (action.intent === null) {
      ...
      // reports itself accurately; for `hold` this deliberately reproduces
      // the exact historical string "hold action selected; no game intent
      // submitted", preserved for byte-stability of recorded artifacts.
      return {
        accepted: true,
  ...
  submitAgentMessage(input: {
    ...
    return this.submitIntent({
      ...
      text: input.text,
      // Generated only inside this trusted in-process runner. It is outside
      // AgentDecision/external policy responses, so policies cannot spoof the
      // evidence join between sender decision, replay, and recipient inbox.
