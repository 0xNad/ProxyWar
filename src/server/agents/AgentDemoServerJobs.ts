...
import { defaultOpenFrontierActiveRosterDir } from "./OpenFrontierNationRegistry";
...
export function loadOpenFrontierHouseAgentBrain(
  env: NodeJS.ProcessEnv = process.env,
): AgentDemoBrain {
  return enumValue(
    env.OPEN_FRONTIER_HOUSE_AGENT_BRAIN,
    houseAgentBrains,
    "planner-codex-cli",
  );
}
      ...
      ? [
          `--agent-manifest-dir=${
            request.roster === "saved"
              ? defaultOpenFrontierActiveRosterDir
              : defaultManifestDir
          }`,
        ]
      : []),
