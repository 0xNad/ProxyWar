...
export interface ExternalAgentRequest {
  ...
  decisionSupport: {
    ...
    parityNote: string;
    /**
     * Present only during the spawn phase. `spawn:<tile>` is accepted for
     * ANY currently-legal tile, not just the offered candidate menu -
     * `reservedTiles` lists every OTHER agent's current spawn reservation
     * (bounded by participant count) and `minDistanceFromReserved` is the
     * distance a requested tile must keep from each. Still validated
     * server-side against live game state before being applied.
     */
    spawnFreeform?: {
      available: true;
      idFormat: "spawn:<tile>";
    ...
    ...(observation.phase === "spawn"
      ? {
          spawnFreeform: {
            ...
            idFormat: "spawn:<tile>" as const,
            tileFormula: "tile = y * map.width + x" as const,
            reservedTiles: observation.spawnReservedTiles ?? [],
            minDistanceFromReserved: observation.spawnMinDistance ?? 0,
          },
        }
      : {}),
  };
