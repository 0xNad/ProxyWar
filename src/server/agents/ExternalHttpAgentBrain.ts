...
import { personalityDiplomacyActionPlayerID } from "./AgentPersonalityDiplomacyPolicy";
...
export interface ExternalAgentRequest {
  ...
  decisionSupport: {
    ...
    /**
     * Non-null only when an offered `alliance_extend` targets an ally that has
     * ALREADY asked to renew. The fact is also on the observation
     * (`visiblePlayers[].allianceOtherAgreedToExtend`), but a policy reading
     * fifteen rivals' worth of JSON will not notice one new boolean, and the
     * window to answer is only 300 ticks wide. Guidance only, like
     * `antiStallHint`; ignoring it is always safe.
     */
    allianceRenewalHint: string | null;
    parityNote: string;
  };
  responseContract: {
...
export function buildExternalAgentDecisionSupport(
  ...
  // Renewal is mutual: the alliance lapses unless BOTH sides ask inside the
  // window, so an offered extend toward an ally that already asked is the one
  // action here that has a deadline and a guaranteed effect.
  const pendingRenewalNames = legalActions
    .filter((action) => action.kind === "alliance_extend")
    .flatMap((action) => {
      ...
      return target?.allianceOtherAgreedToExtend === true ? [target.name] : [];
    });
  return {
    actionIDsByKind,
    ...
    antiStallHint:
      repeatedKind !== null && repeatedCount >= 2
        ...
        : usefulNonHoldActionIDs.length > 0
          ...
          : null,
    allianceRenewalHint:
      pendingRenewalNames.length === 0
        ? null
        : `${pendingRenewalNames.join(", ")} already asked to renew and ${pendingRenewalNames.length === 1 ? "is" : "are"} waiting on you. An alliance is only renewed when BOTH sides ask inside the short window before it expires, so the alliance lapses unless you answer with the offered alliance_extend soon.`,
    parityNote:
