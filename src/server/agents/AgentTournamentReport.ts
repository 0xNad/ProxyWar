...
function tournamentMarkdown(summary: TournamentSummary): string {
  return [
    `# Open Frontier Tournament ${summary.tournamentID}`,
    "",
    "## Overview",
    "",
...
<head>
  ...
  <title>Open Frontier Tournament ${escapeHtml(summary.tournamentID)}</title>
  <style>
...
<body>
  <header><h1>Open Frontier Tournament</h1><p>${escapeHtml(summary.tournamentID)}</p></header>
  <main>
