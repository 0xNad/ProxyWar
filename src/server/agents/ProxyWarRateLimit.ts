export interface ProxyWarRateLimitResult {
  allowed: boolean;
  limit: number;
  remaining: number;
  resetAt: number;
...
export interface ProxyWarRateLimitSnapshot {
  schemaVersion: 1;
  savedAt: number;
  buckets: Array<{
    key: string;
...
export class ProxyWarRateLimiter {
  private readonly buckets = new Map<string, Bucket>();

  constructor(
    private readonly options: {
      windowMs: number;
      initialSnapshot?: ProxyWarRateLimitSnapshot;
      now?: number;
    },
  ) {
  ...
  snapshot(now = Date.now()): ProxyWarRateLimitSnapshot {
...
export function normalizeProxyWarRateLimitSnapshot(
  value: unknown,
  ...
  const buckets = record.buckets
    .map((bucket): ProxyWarRateLimitSnapshot["buckets"][number] | null => {
    ...
    .filter(
      (bucket): bucket is ProxyWarRateLimitSnapshot["buckets"][number] =>
        bucket !== null,
    );
  return {
    schemaVersion: 1,
