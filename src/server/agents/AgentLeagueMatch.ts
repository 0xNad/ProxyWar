...
export class AgentLeagueMatchRunner {
  ...
  async runDecisionTurn(
    ...
    const buildDecisionInputs = () =>
      activeParticipants.map((participant) => {
        ...
        const builtObservation = this.observationBuilder.build(observationInput);
  ...
  private applyCommsSlotSelection(input: {
    ...
    if (result.accepted) {
      this.deliverMessage({
        ...
        message: {
          senderID,
          senderName:
            input.observation.ownState?.name ??
            input.participant.spec.username,
          text: validation.text,
          turnNumber: input.observation.turnNumber,
        },
      });
    ...
    return {
      ...
      commsSlotResult: result.reason.slice(0, MAX_STAMPED_DEAL_REJECTION_LENGTH),
    };
  }
