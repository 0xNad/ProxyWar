...
export function buildAnalyticsReport(
  ...
  const directorCutMilestones: RateMetric[] = [
    { name: "watched_30s" as const, label: "Director Cut: reached 30s" },
    { name: "watched_2m" as const, label: "Director Cut: reached 2 minutes" },
    { name: "watched_50pct" as const, label: "Director Cut: reached 50%" },
    { name: "completed" as const, label: "Director Cut: completed" },
  ].map(({ name, label }) =>
    rateMetric({
      id: name,
      label,
      numerator: totalEventCount(file, name),
      denominator: directorCutStarted,
      methodology: `${label.replace("Director Cut: ", "")} events ÷ director_cut_started events, all-time.`,
    }),
  );

  const mostWatchedEvents = rankingMetric(
    "most_watched_events",
    "Most-watched Featured Events",
    mergedDimensionCounts(file, "director_cut_started", "eventSlug"),
    "director_cut_started events grouped by Featured Event slug, ranked descending, all-time.",
  );
  ...
  const sevenDayReturnRate = rateMetric({
    ...
    label: "Seven-day return rate",
    numerator: returning7d,
    denominator: pageViews7d,
    methodology:
      "(returning_anonymous_visitor + returning_authenticated_visitor) ÷ page_viewed, trailing 7 UTC days.",
  });
