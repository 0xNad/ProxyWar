 ...
 * pinned by tests/server/IsValidSpawnSiteEquivalence.test.ts). Spawn-candidate
 * scans call this once per land tile of the whole map, so the per-call
 * Set/array allocations of `getSpawnTiles` dominated episode start-up
 * memory churn.
 */
