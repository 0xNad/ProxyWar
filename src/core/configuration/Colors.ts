...
/**
 * REPLAY BROADCAST SEAT PALETTE — the 16 territory colours used by the static
 * replay broadcast instead of the stock `humanColors` list below.
 *
 * WHY (aesthetic audit, 2026-08): the broadcast's 16 seats sampled straight
 * off the live canvas came back as raw daisyUI/Tailwind stock values —
 * #3ABFF8, #EC4699, #7C3BED (violet-600), #3C83F6 (blue-500), ... — i.e. the
 * engine's `humanColors` after `clampPlayerColorVisibility`'s HSL round-trip
 * (which is why blue-500 #3B82F6 sampled as #3C83F6). Concrete findings:
 *   - TWO violets (violet-600 next to bright-purple) — indistinguishable
 *     between adjacent territories;
 *   - uniform pastel value across seats — the map read as one flat wash;
 *   - TWO blues nearly identical at map scale — and blue also fights the
 *     ocean (see the ocean rule below).
 *
 * THE VALIDATED PALETTE (properties this constant must preserve):
 *   - ORDER MATTERS: index 0-15 exactly as generated. Seat order is itself a
 *     validated property (alternating light/dark value, hue-walked in OKLCH),
 *     so neighbours in assignment order are also neighbours in max-contrast.
 *   - Closest pair deltaE 0.083 (OKLCH) — every seat pair separable at map
 *     scale.
 *   - All 16 clear of the water/rim/land hazard colours (abyssal ocean
 *     #08162b, shelf water #204c6b, atmos rim #387abd, specular white,
 *     unowned land #403d36, stage #14110f) AND of the reserved hazard coral
 *     #ff6b4a. The coral was missing from that list for a long time, which is
 *     exactly how seat 6 came to sit inside the band the coral reserves — see
 *     the note on that seat below. A reserved colour is a hazard like any
 *     other: if a seat can be mistaken for it, the reservation buys nothing.
 *   - Value-capped at 0.84 (max sRGB channel ~214) — no seat can bloom to
 *     near-white on the lit map.
 *   - NO BLUE BAND: the hue walk stops at ~214deg and restarts at ~272deg.
 *     The OCEAN OWNS BLUE — the largest object on screen is blue water, so
 *     no seat may compete with it.
 *
 * SOURCE OF TRUTH: design/tokens/seats.json, generated by
 * tools/build_palette.py, both in the proxywar-replay repo. The hexes are
 * inlined here only because the engine bundle cannot read that repo at
 * runtime. EDIT THE GENERATOR, NEVER THESE INLINED VALUES — regenerate
 * seats.json and re-inline.
 *
 * Deliberately NOT passed through `clampPlayerColorVisibility`: the palette
 * is already value-capped/validated byte-exact, and the clamp's HSL
 * round-trip would silently shift validated channels (exactly the artefact
 * the audit measured on the stock list).
 */
export const REPLAY_SEAT_COLORS: Colord[] = [
  colord("#7e90d6"), // seat 0  hue 272.0  L 0.76
  colord("#6c58ac"), // seat 1  hue 292.1  L 0.52
  colord("#b781d6"), // seat 2  hue 312.3  L 0.76
  colord("#9f5595"), // seat 3  hue 332.4  L 0.56
  colord("#d674a1"), // seat 4  hue 352.5  L 0.76
  colord("#b2505f"), // seat 5  hue 12.7   L 0.56
  // Seat 6 was #d67763 (L 0.76) and sat INSIDE the reserved hazard coral band:
  // measured at OKLab dE 0.074 from #ff6b4a, below this palette's own 0.075
  // seat-separation floor. The reservation is what lets a coral pixel mean "a
  // warhead did this" with no legend, and this seat can hold 40% of the board
  // in a real match. Root cause: the generator validated every seat against
  // ocean, shelf water, atmos rim, specular white, unowned land and stage —
  // but the hazard coral was never in that hazard set. It is now, and the
  // generator re-solved with the coral cleared: only THIS seat moved (L 0.76
  // -> 0.84), every other seat is byte-identical, closest seat pair still
  // 0.083, coral clearance now 0.118 against a 0.100 floor.
  colord("#d69a8d"), // seat 6  hue 32.8   L 0.84
  colord("#ad5c1d"), // seat 7  hue 52.9   L 0.56
  colord("#d69326"), // seat 8  hue 73.1   L 0.76
  colord("#8b720a"), // seat 9  hue 93.2   L 0.56
  colord("#b2bb37"), // seat 10 hue 113.3  L 0.76
  colord("#55842e"), // seat 11 hue 133.5  L 0.56
  colord("#56cc83"), // seat 12 hue 153.6  L 0.76
  colord("#0e886f"), // seat 13 hue 173.7  L 0.56
  colord("#1dcac8"), // seat 14 hue 193.9  L 0.76
  colord("#106f80"), // seat 15 hue 214.0  L 0.50
];

/**
 * True only inside the static replay broadcast bundle — the same
 * `window.__PROXYWAR_STATIC_REPLAY__` build flag `UserSettings.darkMode()`
 * keys on (../game/UserSettings.ts): set by the broadcast host page before
 * the bundle runs, never in live play, never persisted. `typeof` guard
 * because this module is also loaded where `window` does not exist
 * (server/workers/tests) — there it is always false, i.e. stock behaviour.
 */
export function isStaticReplayBroadcast(): boolean {
  if (typeof window === "undefined") return false;
  const w = window as typeof window & { __PROXYWAR_STATIC_REPLAY__?: boolean };
