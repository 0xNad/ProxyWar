...
function cloneTerrainMapData(data: TerrainMapData): TerrainMapData {
  return {
    nations: data.nations.map((nation) => ({
      ...nation,
      coordinates: [...nation.coordinates],
    })),
    gameMap: cloneGameMap(data.gameMap),
    miniGameMap: cloneGameMap(data.miniGameMap),
    ...(data.teamGameSpawnAreas !== undefined
      ? { teamGameSpawnAreas: cloneTeamGameSpawnAreas(data.teamGameSpawnAreas) }
      : {}),
  };
}

function cloneGameMap(gameMap: GameMap): GameMap {
  const terrain = new Uint8Array(gameMap.width() * gameMap.height());
  gameMap.forEachTile((tile) => {
    terrain[tile] = gameMap.terrainByte(tile);
  });
...
function cloneTeamGameSpawnAreas(
  areas: TeamGameSpawnAreas,
): TeamGameSpawnAreas {
  return Object.fromEntries(
    Object.entries(areas).map(([team, entries]) => [
      team,
