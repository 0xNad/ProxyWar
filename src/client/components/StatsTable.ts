...
import "../graphics/layers/ColumnPicker";
import {
  type ColumnAlignment,
  type ColumnDef,
  columnsFor,
} from "../graphics/layers/lib/StatsColumns";
import {
  COLUMN_IDS,
...
// Column choice persistence. Upstream keeps this on UserSettings, which in
// our fork lives in src/core — core must not import client constants, so
// the (client-only) preference is stored here instead.
const COLUMN_STORE_PREFIX = "settings.statsColumns.";

function storedColumns(kind: StatsTableKind): readonly ColumnId[] {
  try {
    const raw = localStorage.getItem(`${COLUMN_STORE_PREFIX}${kind}`);
    if (raw !== null) {
      const parsed: unknown = JSON.parse(raw);
      if (Array.isArray(parsed)) {
        const valid = parsed.filter((id): id is ColumnId =>
          (COLUMN_IDS as readonly string[]).includes(String(id)),
        );
  ...
  return DEFAULT_STATS_COLUMNS[kind];
}

function storeColumns(kind: StatsTableKind, ids: readonly ColumnId[]): void {
  try {
    localStorage.setItem(`${COLUMN_STORE_PREFIX}${kind}`, JSON.stringify(ids));
  } catch {
...
/**
 * Ported from today's upstream OpenFront (src/client/components/StatsTable.ts):
 * the shared icon-header stats table behind the in-game leaderboard — sortable
 * columns, ⚙️ column picker, virtualized scroll, pinned own-player row.
 */
export abstract class StatsTable extends LitElement {
  ...
  private visibleColumns(): readonly ColumnDef[] {
    const selected = storedColumns(this.tableKind);
  ...
  private onColumnsChanged(event: CustomEvent<ColumnId[]>) {
    storeColumns(this.tableKind, event.detail);
    this.updateStats();
  }
            ...
            >
              ...
              >
                <column-picker
                  ...
                  .selected=${storedColumns(this.tableKind)}
