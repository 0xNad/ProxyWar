...
function accountText(
  ...
  return translateText(`platform_account.${key}`, params, defaultText);
}
 ...
 * list, and individually remove several. The platform is the sole account
 * authority. Works signed out:
...
export class PremiereAccountPage extends LitElement {
  ...
  private async submitClaim(label: string): Promise<void> {
    ...
    try {
      ...
      this.claimError = accountText(
        "save_error",
        "Could not save your claim. Try again.",
      );
    } finally {
  ...
  private async removeClaim(lineageSlug: string): Promise<void> {
    ...
    try {
      ...
      this.claimError = accountText(
        "remove_error",
        "Could not remove that claim. Try again.",
      );
    } finally {
        ...
        <div class="mx-auto flex max-w-3xl flex-col gap-6 px-4 py-8 sm:px-6">
          <header class="flex items-center justify-between gap-3">
            ...
            <h1 class="text-lg font-bold text-ink">
              ${accountText("title", "Your account")}
            </h1>
          </header>
          ${this.renderBody()}
        </div>
  ...
  private renderBody() {
    if (this.loading) {
      return html`<div
        ...
        role="status"
      >
        ${accountText("loading", "Loading your account…")}
      </div>`;
    }
  ...
  private renderIdentity() {
    ...
    const label =
      ...
      accountText("guest_label", `Guest ${identity.accountId.slice(-4)}`, {
        suffix: identity.accountId.slice(-4),
      });
    return html`
      <section
      ...
      >
        ...
        >
          ${accountText("identity_heading", "Identity")}
        </h2>
          ...
          ? html`<div
            ...
            >
              <span>
                ${accountText(
                  "identity_portable",
                  "Linking GitHub makes this identity portable across browsers.",
                )}
              </span>
    ...
    return html`
      ...
      >
        <h2
          ...
          class="text-xs font-semibold uppercase tracking-wide text-ink-muted"
        >
          ${accountText("league_models_heading", "League models")}
        </h2>
        <button
          type="button"
        ...
        >
          ${accountText("claim_cta", "Own a league model? Claim it →")}
        </button>
      </section>
    `;
  }
    ...
    return html`
      ...
      >
        ...
        <p class="text-sm text-ink-muted">
          ${accountText(
            "claim_prompt_body",
            "Do you own one (or more) of the models competing in the league? A person owns a whole lineage of models and policies — claiming any one version claims the whole line, past and future — and can own several lineages. Most visitors don't own one — if that's you, no need to do anything here.",
          )}
        </p>
        <div class="flex flex-wrap items-center gap-2">
          ...
          >
            ${accountText("pick_lineage", "Pick my lineage")}
          </button>
          <button
            type="button"
          ...
          >
            ${accountText("not_owner", "I don't own a model")}
          </button>
        </div>
      </section>
    `;
      ...
      >
        ...
        ${!this.leagueDataLoaded
          ? html`<p class="text-sm text-ink-muted">
              ${accountText("standings_loading", "Loading league standings…")}
            </p>`
  ...
  private renderClaimForm(
    ...
    return html`
      <div class="flex flex-col gap-2">
        ...
        >
          ${accountText("lineage_question", "Which model lineage is yours?")}
        </label>
        <div class="flex flex-wrap items-center gap-2">
          <select
              ...
              ([slug, standing]) => html`
                <option value=${slug}>
                  ...
                  (${standing.policyLabel ?? slug})${standing.isHouse
                    ? accountText("house_suffix", " (house)")
                    : ""}
                </option>
              `,
            )}
          ...
          >
            ${this.savingClaim
              ? accountText("saving", "Saving…")
              : accountText("this_is_me", "This is me")}
          </button>
        </div>
      ...
      >
        <div class="flex items-center justify-between gap-2">
          ...
          <span
            ...
            title=${accountText(
              "unverified_title",
              "This is what you told us — nobody has verified you actually own these model lineages.",
            )}
            >${accountText(
              "unverified_badge",
        ...
        <p class="text-[11px] leading-snug text-ink-muted">
          ${accountText(
            "unverified_body",
            "There is currently no way to prove GitHub-account ownership of a league model, so these claims are private to you and never shown anywhere another player can see them — not on a public profile, a leaderboard, or in any premiere. A verified path arrives with Softmax sign-in.",
          )}
        </p>
        ${this.renderLeagueStaleness()}
        ...
        >
          ${accountText("claim_another", "+ Claim another lineage")}
        </button>
      </section>
    `;
  }
  ...
  private renderClaimedRow(claim: AccountClaim) {
    ...
    const removeButton = html`
      ...
      >
        ${removing
          ? accountText("removing", "Removing…")
          : accountText("remove", "Remove")}
      </button>
    `;
    if (standing === null) {
      return html`
        <div class="flex items-start justify-between gap-3">
          <p class="text-sm text-ink">
            ${accountText(
              "missing_standing",
              `You claimed ${claim.label} (lineage ${claim.lineageSlug}), but it's not in the current league standings mirror — dropped from rotation, renamed, or the mirror is between updates. Your claim is preserved either way.`,
              { label: claim.label, lineage: claim.lineageSlug },
            )}
          </p>
          ${removeButton}
        </div>
      ...
      return html`
        <div class="flex items-start justify-between gap-3">
          <p class="text-sm text-ink">
            ${accountText(
              "house_agent_note",
              `${standing.playerName} is the house agent — the platform's own bot, not a player-owned entry. Its stats aren't shown here.`,
        ...
        <div class="flex flex-1 flex-col gap-2">
          <div class="flex flex-wrap items-baseline justify-between gap-2">
            ...
            <span class="font-mono text-xs tabular-nums text-ink-muted"
              >${accountText(
                "rank_rating",
                `Rank #${standing.rank} · Rating ${standing.score !== null ? standing.score.toFixed(1) : "—"}`,
                {
                  rank: standing.rank,
                  rating:
            ...
            <span>
              ${standing.roundsPlayed !== null
                ? accountText(
                    ...
                    { count: standing.roundsPlayed.toLocaleString() },
                  )
                : accountText("rounds_played_unknown", "Rounds played unknown")}
            </span>
            <span>
              ${accountText(
                "currently_claimed_as",
                `Currently claimed as ${claim.label}`,
                { label: claim.label },
              )}
            </span>
          </div>
  ...
  private renderLeagueStaleness() {
    ...
    const asOf =
      data.lastGoodSyncAt !== null
        ...
        : accountText("unknown_time", "an unknown time");
    return html`
      <p
      ...
      >
        ${accountText(
          "stale_standings",
          `League data is stale — last confirmed ${asOf}. Standings below may be dated.`,
          { time: asOf },
        )}
      </p>
    `;
