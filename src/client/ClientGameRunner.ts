...
function isFullMapReplayRecordingView(): boolean {
  const recordingWindow = window as typeof window & {
    __openFrontFullMapRecording?: boolean;
  };
  const params = new URLSearchParams(window.location.search);
  return (
    recordingWindow.__openFrontFullMapRecording === true ||
    params.has("full-map") ||
    params.get("view") === "full-map"
  );
}
...
export class ClientGameRunner {
  ...
  private hasFocusedReplaySpectator = false;
  ...
  private focusReplaySpectatorOnce() {
    ...
    const focusPlayer =
      ...
      this.gameView
        ...
        .find(
          (player) =>
            player.isAlive() && player.hasSpawned() && player.nameLocation(),
        );
    if (!focusPlayer) {
      return;
    }
    this.hasFocusedReplaySpectator = true;
    this.eventBus.emit(new GoToPlayerEvent(focusPlayer, 8));
  }

  private shouldLockFullMapReplayView(): boolean {
    return (
      this.lobby.gameRecord !== undefined &&
      isAiLeagueReplayRoute() &&
      isFullMapReplayRecordingView()
    );
  }
