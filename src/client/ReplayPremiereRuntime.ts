...
/** Maps the curated War Room kind vocabulary onto the repo's existing `SpectatorEvent.tone` vocabulary (see `AgentSpectatorTelemetry.ts`) so `AnalystEventRow.tone` reads consistently with the Full Replay track's own analyst rows. `plan_change`/`nuke`/`lead_change` are unreachable here (a sealed Premiere never curates any of them — see `pushWarRoomEvent`'s doc; lead_change additionally requires the whole-match state series a sealed Premiere must never fetch) but are mapped for type completeness. */
