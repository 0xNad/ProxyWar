...
export class InGamePromo extends LitElement implements Layer {
  ...
  private destroyBottomRail(): void {
    if (!window.ramp || typeof window.ramp.destroyUnits !== "function") return;
  ...
  private loadAd(): void {
    if (!window.ramp || typeof window.ramp.spaAddAds !== "function") {
  ...
  public hideAd(): void {
    ...
    if (!window.ramp || typeof window.ramp.destroyUnits !== "function") {
      console.warn("Playwire RAMP not available for in-game ad");
      return;
    }
    this.shouldShow = false;
