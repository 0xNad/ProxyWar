...
export class LocalServer {
  ...
  // once true, automatic pacing (the archived-replay fastest-default) stops
  ...
  start() {
    ...
    this.eventBus.on(ReplaySpeedChangeEvent, (event) => {
      ...
      if (event.source === "user") {
        // House rule (P0 incident, 2026-08-03): once a viewer has manually
        // picked a speed, automatic pacing (the archived-replay
        // fastest-default) must never silently revert it -- a default,
        // never a lock. This stays set for the rest of the session; a
        // fresh page load (Reset, a backward jump-to-turn navigation)
        // starts a new LocalServer instance and clears it naturally.
