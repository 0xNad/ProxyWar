...
// A dispatched-but-not-yet-rendered gap (`this.turns.length -
// this.turnsExecuted`) beyond this size can only mean a genuine catch-up
// stall, never ordinary fastest-speed steady state, which never lets the
// backlog exceed MAX_REPLAY_BACKLOG_TURNS above.
export const AI_LEAGUE_REPLAY_CATCHUP_BACKLOG_THRESHOLD =
  MAX_REPLAY_BACKLOG_TURNS * 5;
// A catch-up that resolves inside one worker round-trip must never reach
// the viewer -- see reportReplayCatchUp's own doc.
export const AI_LEAGUE_REPLAY_CATCHUP_DEBOUNCE_MS = 200;
// Real turn-count catch-up progress for archived Full Replay. `detail` is
// `{ turnsRendered, turnsTotal }` while genuinely behind the dispatched
...
export class LocalServer {
  ...
  /**
   * Main.ts's `join.then()` handler emits `ReplaySpeedChangeEvent(fastest)`
   * on every ai-league-replay open so archived Full Replay never defaults
   * to real-time (1x) playback. That emission races this class's own
   * construction: `lobbyHandle.join` (ClientGameRunner.ts's `joinLobby()`
   * `resolveJoin()`) resolves the instant the "start" message arrives --
   * synchronously BEFORE `createClientGame()`'s async work (terrain load,
   * layer construction) finishes and `ClientGameRunner.start()` ->
   * `Transport` -> `LocalServer.start()` ever reaches the
   * `ReplaySpeedChangeEvent` listener registered a few lines below.
   * `EventBus.emit()` has zero buffering for a not-yet-registered
   * listener, so the event can be silently dropped, leaving
   * `replaySpeedMultiplier` at its field-initializer default (`normal`,
   * real-time). Applying the identical rule directly here removes the
   * dependency on emit/subscribe ordering instead of trying to win the
   * race.
   *
   * NOTE for the next reader chasing a "replay looks frozen" report: this
   * fix closes the ordering race, but it is NOT the dominant cause of a
   * slow-looking replay open. Director Cut is enabled by default for
   * AI League Full Replay and re-asserts its OWN per-segment speed via
   * `mountDirectorCutController`'s `onSpeedChange` the moment its plan
   * hydrates (`AiLeagueReplayOverlay.ts`) -- confirmed live: a "normal" or
   * "slow" opening segment overrides whatever this method just set, within
   * ~200ms, for as long as that segment covers the current turn. That is
   * Director Cut's own deliberate narrative pacing (a "normal"-paced
   * establishing shot before speeding through quieter turns), generated by
   * `DirectorCutPlan`, not a defect in this class or its turn-dispatch
   * pipeline -- changing which segments get which speed is a content/
   * narrative-pacing decision, not a client bug fix. This method's actual
   * effect is narrower: the brief pre-hydration window before Director
   * Cut's plan applies, and the total fallback when a plan is absent or
   * fails to load. See GameRightSidebar.ts's speed-label change (same
   * incident) for the honest, always-visible indicator of the pace
   * Director Cut is actually holding, so that pace reads as intentional
   * rather than frozen.
   *
   * A URL-forced speed (`parseReplayRenderSpeed`, used by headless clip
   * capture) or an active Clip Preview pause is a deliberate override and
   * must still win.
   */
  private applyArchivedReplayDefaultSpeed(): void {
    if (
      this.lobbyConfig.gameRecord === undefined ||
      this.lobbyConfig.replayClipPreviewTarget !== undefined ||
      parseReplayRenderSpeed(
        typeof window === "undefined" ? "" : window.location.search,
      ) !== null
    ) {
  ...
  /**
   * A visible "catching up" gap is real only while the client has
   * dispatched turns to the worker (`this.turns.length`) that haven't come
   * back rendered yet (`this.turnsExecuted`, incremented once per
   * ClientGameRunner.ts `transport.turnComplete()`/`turnsComplete()` call
   * on every non-premiere game_update) -- the exact two counters that
   * already define `allowReplayBacklog` in the pacing loop above. Two
   * distinct situations open this same gap: the initial dispatch-before-
   * first-render window right after `start()`, and a big
   * `ReplayJumpToTurnEvent` seek re-opening it mid-match. Both share this
  ...
  private reportReplayCatchUp(): void {
    ...
    const backlog = Math.max(0, this.turns.length - this.turnsExecuted);
    const shouldCatchUp =
      (this.turnsExecuted === 0 && this.turns.length > 0) ||
      backlog > AI_LEAGUE_REPLAY_CATCHUP_BACKLOG_THRESHOLD;
    if (!shouldCatchUp) {
      this.catchUpPendingSinceMs = null;
      if (this.reportedCatchingUp) {
        this.reportedCatchingUp = false;
        document.dispatchEvent(
          new CustomEvent(AI_LEAGUE_REPLAY_CATCHUP_EVENT, { detail: null }),
        );
      }
      return;
    }
    this.catchUpPendingSinceMs ??= Date.now();
    if (
      Date.now() - this.catchUpPendingSinceMs <
