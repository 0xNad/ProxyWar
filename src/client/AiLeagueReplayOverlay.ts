...
export function mountAiLeagueReplayOverlay(input: {
  ...
  banner.textContent = "Replay mode: watching Open Frontier agents";
...
function overlayHtml(input: AiLeagueReplayOverlayInput): string {
  ...
  const setupLine =
    agentCount > 0 || configuredOpponentCount > 0
      ? `${agentCount} Open Frontier agents vs ${configuredOpponentCount} built-in opponents`
      : "Open Frontier agents vs built-in opposition";
  const configLine = [
  ...
  return `
    ...
    <div class="ai-league-body">
      ...
      <p class="ai-league-muted">This uses the real Open Frontier replay renderer. The viewer has no player identity and replay intents are read-only.</p>
      <p>
