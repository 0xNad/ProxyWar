...
raw telemetry as the default view; agent/builder directories and profiles
...
| `/premiere/:premiereId`       | `index.html` game shell                                             | sealed live, or archived                                     |
| `/ai-league-replay/:runID`    | `index.html` game shell                                             | Full Replay / Analyst mode                                   |
...
## 7. Security/privacy audit results
...
## 8. Test/build results (this session, full matrix)
...
`test.todo` (premiere reveal-after-end — see §11); `npm test`'s 3 todo are
pre-existing, unrelated to this overhaul.

## 9. Deployed URL + smoke results, or the exact gate/blocker
...
- Post-deploy smoke checklist is the literal spec §9.3 list, walked
  locally against the fixture server this session (§7 above + the
  screenshot capture in §10) with one addition: the `X-Forwarded-For`
...
## 10. Before/after screenshot paths
...
2. **Builder dashboard**: not built, documented gap with the exact missing
   dependency list (§6). Real work, not urgent — the public Builder
   identity + `/build` flow are complete without it.
3. **Real GitHub-sign-in-to-Softmax-control verification**: unsolved by
...
4. **`origin/main` drift**: 3 commits (`1855d5575`/`4b66af87b`/`6f366d8ed`)
...
5. **Storage remediation (Stage 8B)**: never executed — remains fully
   operator-gated per its own spec section. Not currently blocking (see
   §8's disk-floor note), but the inventory/proposal/approval process
   itself was never run.
6. `main-bzYr7PX1.js` (3.17 MB uncompressed, 850 KB gzip) exceeds Vite's
