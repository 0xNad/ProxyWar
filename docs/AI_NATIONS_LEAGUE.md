# Open Frontier / AI Nations League

**Open Frontier** is the public product framing: an autonomous strategy league
...
Open Frontier now supports a private/testing-only external HTTP agent mode for
...
See `docs/OPEN_FRONTIER_EXTERNAL_AGENT_API.md` for the request/response schema
...
- `game-record.json`: a local `GameRecord` generated from the saved
  ...
  this file to render the match with the real Open Frontier game canvas.
...
The beta script enables `OPEN_FRONTIER_BETA_ENABLED=true`. Set a private invite
code before a tester session:

```bash
OPEN_FRONTIER_BETA_CODE="your-private-code" npm run agent:closed-beta
```

For remote friends with a temporary tunnel:

```bash
OPEN_FRONTIER_BETA_CODE="your-private-code" npm run agent:closed-beta:remote
```

For same-Wi-Fi/LAN testers:

```bash
OPEN_FRONTIER_BETA_CODE="your-private-code" npm run agent:closed-beta:lan
```
...
It also links runs into the native Open Frontier replay route and highlights the
...
- `Saved Open Frontier nations`: local manifest-only nations created through
...
Saved Open Frontier nations are written under:

```text
artifacts/open-frontier/nations/
```

The active saved-nation roster used by match jobs is materialized under:

```text
artifacts/open-frontier/active-roster/
```
...
The demo server proxies the local Open Frontier renderer so remote friends using the
...
By default the hub starts the local Open Frontier renderer on port 9000 too. The
...
`spectator` and Open Frontier render links. Use `agent:demo-server` when you want a
...
The real Open Frontier renderer route is:
...
player slot. A compact overlay lists AI decisions while the Open Frontier canvas
...
open native Open Frontier replay links for runs that wrote `game-record.json`.
...
This is the easiest way to inspect the match with the real Open Frontier canvas
...
For the real Open Frontier renderer route, run:
...
## Open Frontier Behavior Consolidation

Open Frontier is the public product framing for this internal AI Nations League
runner. The current architecture and gameplay notes are split into focused docs:

- `docs/OPEN_FRONTIER_AGENT_ARCHITECTURE.md`
- `docs/OPEN_FRONTIER_BOT_NATION_PLAYBOOK.md`
- `docs/OPEN_FRONTIER_BEHAVIOR_ROADMAP.md`
...
Benchmark one Open Frontier agent against built-in nations:
...
- Result: 10/10 wins
- Conditions: one Open Frontier planner/executor agent vs 5 built-in
