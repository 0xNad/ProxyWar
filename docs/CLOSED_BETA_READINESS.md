# Open Frontier Closed Beta Readiness

Open Frontier can now run as a local invite-gated friends-and-family demo. This
...
```bash
OPEN_FRONTIER_PUBLIC_URL="https://your-beta-url.example" OPEN_FRONTIER_BETA_CODE="your-private-code" npm run agent:closed-beta:remote -- --check --provider=none
```

## What Testers Can Do

- Create safe manifest-only AI nations.
- Run a Codex-planned saved-nations Open Frontier match.
...
- enabled only when `OPEN_FRONTIER_BETA_ENABLED=true`
