...
| Donations: require `isFriendly` (ally or team), both alive, recipient not disconnected; per-recipient 10s cooldown; troops default 1/3 of sender capped by recipient headroom; **gold null-amount also defaults to 1/3 of the sender's gold — fixed 2026-08-07** (it was a dead default: `toInt(goldNum ?? 0)` at line 33 coerced null to 0, so the `??=` fallback at line 49 was unreachable and a null-amount gold donation silently no-opped; null is now preserved through construction and the fallback fires). Gold has no recipient-side cap — `removeGold` clamps the transfer to the sender's balance. Donations to Nations bypass the human-only config toggles. No donation-request mechanic exists anywhere (quick-chat "help" keys are cosmetic) | `PlayerImpl.ts:738-790, 969-976`, `DonateGoldExecution.ts:21, 33, 49`, `DonateTroopExecution.ts:44-51`, `DefaultConfig.ts:553-558`, `Schemas.ts:384-394` **[gap]** |
...
dead-default was fixed on 2026-08-07 — null now defaults to 1/3 of the sender's
gold, `tests/DonationRules.test.ts` pins the new default, and
`tests/DonateGoldDefaultAmount.test.ts` pins the explicit-amount, explicit-zero
and sender-balance-clamp edges; the
