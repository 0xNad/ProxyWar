# Open Frontier Remote Friends Beta

This guide is for sharing a local Open Frontier beta with trusted friends or
family.
...
```bash
OPEN_FRONTIER_BETA_CODE="make-a-private-code" npm run agent:closed-beta:remote
```
...
```bash
OPEN_FRONTIER_BETA_CODE="make-a-private-code" npm run agent:closed-beta:remote -- --check
```
...
```bash
OPEN_FRONTIER_PUBLIC_URL="https://your-beta-url.example" OPEN_FRONTIER_BETA_CODE="make-a-private-code" npm run agent:closed-beta:remote -- --check --provider=none
```

When the tunnel prints a `trycloudflare.com` URL, share:
...
```bash
OPEN_FRONTIER_BETA_CODE="make-a-private-code" npm run agent:closed-beta:remote -- --provider=none
```
...
```bash
OPEN_FRONTIER_PUBLIC_URL="https://your-beta-url.example" OPEN_FRONTIER_BETA_CODE="make-a-private-code" npm run agent:closed-beta:remote -- --provider=none
```
...
```bash
OPEN_FRONTIER_BETA_CODE="make-a-private-code" npm run agent:closed-beta:lan
```
...
- There is no built-in default invite code. Always set
  `OPEN_FRONTIER_BETA_CODE` before sharing a remote link.
...
- automatic transfer into the rendered Open Frontier replay when a match completes
- a longer default spectacle match: saved Open Frontier nations plus five
  built-in nations and five tribes/bots on a compact Pangaea map
...
- paste `/open-frontier/decide` into manual **Test Endpoint**
...
The endpoint never sends raw game intents. Open Frontier validates the
selected id and still submits through `AgentRunner -> GameServer`. Saved
external endpoints are part of the local saved-nations queue; curated defaults
fill the roster until at least four entrants exist.
...
`artifacts/open-frontier/secrets/` and stores only `tokenSecret` in the manifest.
...
```bash
OPEN_FRONTIER_ALLOW_PRIVATE_AGENT_ENDPOINTS=true OPEN_FRONTIER_BETA_CODE="make-a-private-code" npm run agent:closed-beta
```
...
saved-roster match uses up to eight saved entrants. If fewer than four are
queued, curated defaults fill the empty slots.
...
- The rendered Open Frontier view is currently a replay that starts as soon as the
...
- Match jobs are bounded by a local one-at-a-time runner. The remote helper
  ...
  not stack long runs.
- The visible Open Frontier renderer uses the native replay controls and now
...
For a full benchmark-style match against built-in nations:
...
The friend-facing beta page uses this Codex planner path by default with
`gpt-5.4`, medium reasoning, six strategy rounds, five built-in nations, five
tribes/bots, compact Pangaea, and varied spawn candidates. Non-LLM house-agent
brains are not part of the tester flow.
...
```bash
AI_LEAGUE_CODEX_COMMAND="/Applications/Codex.app/Contents/Resources/codex" OPEN_FRONTIER_BETA_CODE="make-a-private-code" npm run agent:closed-beta:remote
```
...
```bash
OPEN_FRONTIER_PUBLIC_URL="https://your-beta-url.example" OPEN_FRONTIER_BETA_CODE="make-a-private-code" npm run agent:public-readiness:strict
```
...
- rendered Open Frontier replay route
- AI decision overlay
