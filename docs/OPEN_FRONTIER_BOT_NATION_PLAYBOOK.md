# Open Frontier Bot And Nation Playbook

Last updated: 2026-05-11

This document summarizes built-in nation and tribe behavior so Open Frontier agents can learn from it without moving non-deterministic agent logic into core.
...
Open Frontier agents now expose build-placement metadata on build actions:
...
The built-in nation code is deterministic core simulation behavior. Open Frontier agents should borrow concepts, not move LLMs or product agent logic into core. Agent improvements should stay under `src/server/agents`, scripts, tests, docs, and artifacts.
