...
```bash
PROXYWAR_ALLOW_PRIVATE_AGENT_ENDPOINTS=true PROXYWAR_BETA_CODE="choose-a-private-code" npm run agent:closed-beta
```

For remote friends, use the remote helper:

```bash
PROXYWAR_BETA_CODE="choose-a-private-code" npm run agent:closed-beta:remote
```
...
```bash
PROXYWAR_PUBLIC_URL="https://your-beta-url.example" PROXYWAR_BETA_CODE="choose-a-private-code" npm run agent:public-readiness:strict
```
...
Use `docs/PROXYWAR_HOSTED_BETA.md` for the deploy checklist and `deploy/`
...
starter agent command, run it, paste `http://127.0.0.1:7777/proxywar/decide`,
...
Before sharing, check saved external agents from the terminal:
...
- for local endpoints, start with `PROXYWAR_ALLOW_PRIVATE_AGENT_ENDPOINTS=true`
