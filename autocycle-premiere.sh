...
# Extra bounded wait, ON TOP OF the grace window above, for the durable
# settlement-ledger record to appear before cycling wipes the state root
# that's the only other place the winner is known. Never blocks
# indefinitely - see the loud "cycling anyway" warning below, which is
# the actual fix for the 2026-08-02 silent-loss incident (a real,
# unattended, natural settlement whose settlement-ledger write never
# landed - see docs/project-state/known-problems.md): the record's
# absence is now visible in this log instead of only discoverable by
# disk archaeology after the fact.
SETTLEMENT_CONFIRM_MAX_WAIT_SECONDS="${AUTOCYCLE_SETTLEMENT_CONFIRM_MAX_WAIT_SECONDS:-60}"
...
# True once the durable settlement-ledger record exists for premiere $1 -
# the same signal `PremiereEndedPage.ts`'s ended-page Winner line and
# `GET /api/premieres/<id>/settlement` depend on surviving the state-root
# wipe below.
settlement_confirmed() {
    curl -s -o /dev/null --fail -m 10 "${LOCAL_ORIGIN}/api/premieres/$1/settlement" 2> /dev/null
}
    ...
    case "$status" in
        settled | void)
            ...
            if [ -z "$settled_since" ]; then
                ...
                if settlement_confirmed "$premiere"; then
                    cycle
                    settled_since=""
                    settlement_wait_since=""
                elif [ -z "$settlement_wait_since" ]; then
                    settlement_wait_since="$now"
                    log "premiere ${premiere} grace elapsed but settlement-ledger record not yet confirmed - waiting up to ${SETTLEMENT_CONFIRM_MAX_WAIT_SECONDS}s more before cycling anyway"
                elif [ "$((now - settlement_wait_since))" -ge "$SETTLEMENT_CONFIRM_MAX_WAIT_SECONDS" ]; then
                    log "!! premiere ${premiere} settlement-ledger record STILL not confirmed after ${SETTLEMENT_CONFIRM_MAX_WAIT_SECONDS}s extra wait - cycling anyway (its ended page and /settlement API will 404; check /tmp/pw-bet-origin.log for settlement_ledger_write_failed)"
                    cycle
                    settled_since=""
                    settlement_wait_since=""
                fi
