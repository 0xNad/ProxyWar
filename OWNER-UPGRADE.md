...
| Spatial/minimap  | Canonical package absent/default-off; canary pending | Require `observation.spatial.schemaVersion === 1` **and** `visibilityModel === "global-lockstep-public-map-v1"`. Accept only the documented bounded v1 shape. Use it to rank the current `legalActions`; output only the chosen offered ID. Behavior is byte-compatible when absent.                                                               |

“Spatial/minimap v1” is deliberately narrow: own shape/centroid, bounded rival
bearing/border relations including a coarse land/coastal/mixed frontier class,
and a 24x12 ownership minimap. It does not
contain a coordinate frame, elevation, structure positions, unit positions, or
warship positions, so this packet does not call it the full rich-spatial-v2
design.
...
`a8ffed6115d7b848487ddb5b935f191c20e1c740` / tree
`eb63ce3df2003ebea905b05e4ce191f42932d354`. The packet verification script
...
message event alongside it. The checker rejects raw body/prompt/provider fields,
unknown fields, lines over 8 KiB, logs over 16 MiB, and more than 64 events of
one kind per policy log. The body itself is never written by the reference
evidence logger.

The canonical `proxywar:0.1.54` target proves deals and free text. Do not claim
spatial from it: spatial is absent there. Until a reviewed spatial Coworld ID is
published in the evidence table, owners can test spatial only locally and must
retain absent-field fallback.
...
| Spatial/minimap   | Independently reviewed bounded-v1 lane `b7b8c03acfdd4509ba9cee7aac34ae0407c58e70`, integrated candidate `a8ffed6115d7b848487ddb5b935f191c20e1c740`; no Coworld/policy/request identity yet                                       | Independently fetched immutable Coworld authority receipt; bounded wire/prompt/memory; exact offered gameplay ID; matched off/on episodes with identical non-spatial config                                    | **LOCAL PASS; HOSTED BLOCKED:** clean integrated benchmark and 4/8/16/25 prompt matrix both `targetMet=true`; acceptance p95 13.43 ms, minimap 1,132 B, retained-on 0 B; real 25-seat boundary 1,635 B. Artifact SHA-256 values are in `FRESH-APPLY.json`. No authority receipt, package, or XP exists, so no gameplay/live claim. Rich v2 remains unimplemented. |
| Fresh owner apply | public base `190ea95eda41fbf5d1521d433b3365d87b9cfe57`; candidate `a8ffed6115d7b848487ddb5b935f191c20e1c740`; exact patch and before/after hashes in `coworld-adapter/testing/owner-upgrade/SHA256SUMS`                          | Exact ledger cardinality, candidate commit/tree/blobs, `git apply --check`, complete-file hashes, install, Node tests, and syntax from a fresh exact-base checkout; doctor is a separate environment preflight | **LOCAL PASS:** verifier returned `PASS` for all 13 required files; patch SHA-256 `3cd39820626d87229aae0795e5bf470f0fcb3da5cdbbb5444ebff8d9be87f019`. Upload, doctor readiness, and owner XP remain separate owner/operator actions and are not claimed by this packet.                                                                                           |
...
| Spatial absent, `schemaVersion !== 1`, or visibility model not exact   | Legacy/disabled/unknown or non-public provenance    | Ignore spatial and run the prior action ranking byte-compatibly.                                                                                                    |
