...
bounded completed structure/warship positions, weighted rival borders,
encirclement, naval exposure, and a separately flagged 24x12 or adaptive 32x16
ownership/terrain/marker minimap. Schemas `1` and `3` are retained only as
bounded backward-compatible consumers.
...
Spatial minimap rows are exactly 12x24 or 16x32 allowlisted glyphs when the
child flag is enabled. The normalized minimap must be at most 4 KiB;
...
optional minimap does not discard an otherwise valid schema-5 map/front/asset
...
record and every such record to be present, schema `5`, minimap-schema-`2`, and
...
| Spatial/map       | **PENDING RESEAL:** schema-5 source, clean benchmark, 4/8/16/25-seat prompt matrix, Coworld/policy/request identities, and hashes must be inserted only after the final bytes freeze                                                                      | Independently fetched immutable Coworld authority receipt; bounded wire/prompt/memory; exact offered gameplay ID; matched off/on episodes with identical non-spatial config           | **LOCAL DIRTY SUCCESSOR ONLY; HOSTED BLOCKED:** schema `5` now implements L1-L5, but the clean source-attributed benchmark, regenerated owner packet, package authority receipt, and hosted XP do not yet exist. Do not reuse the prior schema-3 hashes as schema-5 evidence.                                                                                 |
...
| Spatial absent, unknown schema, or visibility model not exact                          | Disabled/unknown or non-public provenance           | Ignore spatial and run the prior action ranking byte-compatibly. Schemas `1`/`3` are bounded fallbacks; full rich fields require exact schema `5`.                  |
| Bad map frame, coordinate, terrain/coverage, asset, graph, naval, owner, count, or cap | Malformed rich spatial container                    | Reject the entire schema-5 spatial block and all derived rival fields/notes. Never clamp, crop, infer, or partially retain it.                                      |
| Malformed 24x12/32x16 minimap, terrain row, marker, identity, or glyph                 | Invalid optional child                              | Omit the entire minimap from policy state; do not pad, crop, infer tiles, or downgrade schema `5` to minimap schema `1`.                                            |
