...
const port = Number(process.env.OPEN_FRONTIER_AGENT_PORT ?? "7777");
const decisionPath =
  process.env.OPEN_FRONTIER_AGENT_ENDPOINT_PATH ?? "/open-frontier/decide";
...
http
      ...
      response.end(
        JSON.stringify(
          createHealthResponse({
            ...
            cardPath,
            agentName:
              process.env.OPEN_FRONTIER_AGENT_NAME ?? "Frontier SDK Agent",
          }),
        ),
      );
      return;
    ...
    if (request.method === "GET" && requestUrl.pathname === "/") {
      ...
      response.end(
        JSON.stringify({
          ...
          message: "Paste agentCardUrl into Open Frontier Connect With One Link.",
        }),
      );
      return;
    }
  ...
  .listen(port, host, () => {
    console.log(
      `Open Frontier LLM starter agent listening at http://${host}:${port}${decisionPath}`,
    );
