...
//      evenly spaced. 48 engages before the observed crash window, so snapshot
//      heap is flat through the mid-game depths where the pod OOMs. The
//      spectator replay then carries up to ~48 evenly-spaced snapshots (a
//      watchability trade the operator can raise via the env override); the
//      rendered replay (message-derived game-record) is unaffected.
// Env-overridable via PROXYWAR_MAX_RETAINED_SNAPSHOTS or
// COWORLD_MAX_RETAINED_SNAPSHOTS (floored at 16).
const COWORLD_MAX_RETAINED_SNAPSHOTS = Math.max(
  16,
  Number(
    process.env.PROXYWAR_MAX_RETAINED_SNAPSHOTS ??
      process.env.COWORLD_MAX_RETAINED_SNAPSHOTS ??
      "48",
  ),
);
