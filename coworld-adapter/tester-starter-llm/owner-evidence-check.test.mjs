...
function validEvents() {
  return [
    ...
    },
    {
      ...
      messageEventID: "msg_33333333-3333-4333-8333-333333333333",
      senderID: "P_A",
      senderTurn: 42,
      messageBodySHA256: digest,
      messageBodyUTF8Bytes: Buffer.byteLength(body, "utf8"),
...
test("owner evidence checker requires unique server-owned message event ids", () => {
  const missing = validEvents();
  delete missing[3].messageEventID;
  const missingResult = runChecker(missing);
  assert.equal(missingResult.status, 1);
  assert.match(missingResult.stderr, /missing a server-owned event id/u);

  const reused = validEvents();
  reused.push({
    ...structuredClone(reused[3]),
    requestID: "req_recipient_later",
  });
  const reusedResult = runChecker(reused);
  assert.equal(reusedResult.status, 1);
  assert.match(reusedResult.stderr, /reused a server-owned event id/u);
});
