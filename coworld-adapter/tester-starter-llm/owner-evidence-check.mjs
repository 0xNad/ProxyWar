  ...
  if (event.kind === "deal_selection") {
    ...
    if (
      (Object.hasOwn(event, "messageEventID") &&
        !isBoundedString(event.messageEventID)) ||
...
function parseArgs(argv) {
  ...
  if (
    ...
    fail(
      "usage: node owner-evidence-check.mjs --deals=required|optional --messages=required|optional --spatial=absent|present|rich-v3|rich-v3-minimap|rich-v5|rich-v5-minimap|either LOG [LOG ...]",
    );
  }
...
function joinedMessageChecks(events) {
  ...
  if (
    observations.some(
      (observation) => !isBoundedString(observation.messageEventID),
    )
  ) {
    fail("message observation evidence is missing a server-owned event id");
  }
  if (
    new Set(observations.map((observation) => observation.messageEventID))
      .size !== observations.length
  ) {
    fail("message observation evidence reused a server-owned event id");
  }
